...
Атрибуты на позиционных параметрах `record` пишутся **прямо на параметре**, без `[property: ...]`.
Ранняя редакция этого раздела предписывала обратное — это оказалось неверно и проверено на практике:
на .NET 8 атрибут, перенесённый на свойство, приводит к `InvalidOperationException` при биндинге
(«validation metadata must be associated with the constructor parameter»), и падают все запросы к
эндпоинту. Обнаружено прогоном `ServicesTests`, обоснование продублировано комментарием в
`ServiceDto.cs`. Деталь неочевидная; проверяется тестом `SVC-015` (`durationMinutes = 0` → 400),
а не глазами.
...
**Уточнение, найденное при реализации:** `WriteAsJsonAsync` в простой перегрузке перезаписывает
`Content-Type` на `application/json`, даже если заголовок выставлен заранее. Поэтому вызывать нужно
перегрузку с явным параметром `contentType: "application/problem+json"`. Тест проверяет именно
заголовок ответа, а не только код, чтобы это не откатили обратно.
