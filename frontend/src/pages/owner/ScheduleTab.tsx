    ...
    <Modal title={title} onClose={onClose}>
      <div className="flex flex-col gap-5">
        ...
        <label className="flex items-center gap-3 cursor-pointer select-none">
          ...
          >
            <div
              className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${isWorking ? 'translate-x-6' : 'translate-x-1'}`}
            />
          </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {breaks.map((b, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 bg-warning-bg border border-[#EAD9AC] rounded-xl px-3 py-2"
                  >
        ...
        {mutError && <p className="text-sm text-danger text-center">{mutError}</p>}
      </div>
    </Modal>
  )
}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-7 mb-1">
        {WEEK_DAYS.map((d) => (
          <div key={d} className="text-center text-[11.5px] font-medium text-muted py-1">
            {d}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {days.map((day) => {
          ...
          let cellClass =
            'relative flex flex-col items-center justify-start pt-1.5 pb-1 rounded-[10px] h-[52px] text-sm cursor-pointer transition-all select-none border '
          ...
          return (
            <div key={key} className={cellClass} onClick={() => !past && onDayClick(day)}>
              ...
              {entry && !entry.isWorking && <span className="text-[9px] text-muted">выходной</span>}
            </div>
          )
        })}
      </div>
...
export function ScheduleTab({ companyId, selfMasterId }: Props) {
  ...
  const masters = selfMasterId
    ? []
    : (members?.filter((m) => m.role === 'Master' || m.role === 'CompanyOwner') ?? []).sort((a, b) => {
  ...
  return (
    <div>
      ...
      {showTemplate && masterId && (
        <WeeklyTemplateModal masterId={masterId} companyId={companyId} onClose={() => setShowTemplate(false)} />
      )}
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {masters.map((m) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center font-semibold text-[10.5px] shrink-0 ${
