...
function CompanyChips({ companies, companyId, onSelect }: { companies: { id: string; name: string }[]; companyId: string; onSelect: (id: string) => void }) {
...
function MyCompaniesTab() {
  ...
  return (
    <div>
      ...
      {isLoading ? (
        <div className="flex flex-col gap-3">{Array.from({length:2}).map((_,i)=><div key={i} className="h-24 bg-cream-deep rounded-2xl animate-pulse"/>)}</div>
      ) : companies && companies.length > 0 ? (
        <div className="flex flex-col gap-3">
          {companies.map(c => (
            <div key={c.id} className="bg-white border border-line rounded-[18px] px-[22px] py-[18px] flex items-center justify-between gap-3.5 flex-wrap">
              ...
              <div className="flex gap-2">
                ...
                <Link to={`/company/${c.slug}`} target="_blank" className="inline-flex items-center px-4 text-[13px] font-semibold text-ink-soft hover:text-ink">Открыть →</Link>
              </div>
            </div>
          ))}
        </div>
        ...
        <Modal title="Создать компанию" onClose={() => { setShowCreate(false); reset() }}>
          <form onSubmit={handleSubmit(d => create.mutate({ name:d.name, slug:d.slug||slugify(d.name), description:d.description||undefined, address:d.address||undefined, phone:d.phone||undefined, email:d.email||undefined, allowSelfBooking:d.allowSelfBooking, showInPublicListing:d.showInPublicListing }))} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea rows={2} className="rounded-xl border border-line px-4 py-3 text-sm outline-none focus:border-gold focus:ring-[3px] focus:ring-cream-deep resize-none bg-white text-ink" {...register('description')} />
            </div>
...
function ScheduleSection() {
  ...
  if (!companies?.length) return <Card className="p-12 text-center text-muted"><Icon name="store" size={32} strokeWidth={1.4} className="mx-auto mb-2" /><p>Вы не привязаны ни к одной компании</p></Card>
...
function DashboardSection() {
  ...
  if (isLoading) return <div className="h-40 bg-cream-deep rounded-2xl animate-pulse" />
  if (!companies?.length) return <Card className="p-12 text-center text-muted"><Icon name="store" size={32} strokeWidth={1.4} className="mx-auto mb-2" /><p>У вас нет компаний</p></Card>
    ...
    <div>
      <div className="flex flex-wrap gap-3 mb-5 items-end">
        {companies && companies.length > 1 && (
          <select value={selectedId} onChange={e => setCompanyId(e.target.value)} className="rounded-xl border border-line px-3.5 py-2.5 text-sm outline-none focus:border-gold bg-white text-ink">
            {companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
        )}
        <div className="flex flex-col gap-1">
          <label className="text-xs text-muted">С</label>
          <input type="date" value={from} onChange={e => setFrom(e.target.value)} className="rounded-xl border border-line px-3.5 py-2.5 text-[13.5px] outline-none focus:border-gold bg-white text-ink" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs text-muted">По</label>
          <input type="date" value={to} onChange={e => setTo(e.target.value)} className="rounded-xl border border-line px-3.5 py-2.5 text-[13.5px] outline-none focus:border-gold bg-white text-ink" />
        </div>
        <Button onClick={() => refetch()} disabled={!selectedId}>Построить отчёт</Button>
      </div>

      {isLoading ? (
        <div className="flex flex-col gap-2.5">{Array.from({length:3}).map((_,i)=><div key={i} className="h-20 bg-cream-deep rounded-2xl animate-pulse"/>)}</div>
      ) : data ? (
        <>
          {data.length === 0 ? (
            ...
            <>
              <div className="flex flex-col gap-2.5 mb-3.5">
                {data.map(r => (
                  <div key={r.masterId} className="bg-white border border-line rounded-2xl px-5 py-4 flex items-center justify-between gap-4 flex-wrap">
                    <div>
                      <p className="font-semibold text-sm text-ink">{r.masterName}</p>
                      <p className="text-[12.5px] text-muted mt-0.5">{r.bookingsCount} услуг · комиссия {r.commissionPercent}%</p>
                    </div>
                    <div className="text-right text-[13px]">
                      <p className="text-ink-soft">Итого: <strong className="text-ink">{r.totalAmount.toLocaleString('ru-RU')} ₽</strong></p>
              ...
              <div className="bg-cream-deep rounded-2xl px-5 py-4 flex justify-between font-semibold text-ink flex-wrap gap-2">
                ...
                <div className="text-right text-[13px]">
                  <p className="text-ink font-semibold">{data.reduce((s,r)=>s+r.totalAmount,0).toLocaleString('ru-RU')} ₽ всего</p>
                  <p className="text-success">{data.reduce((s,r)=>s+r.masterEarnings,0).toLocaleString('ru-RU')} ₽ мастерам</p>
                </div>
              </div>
            </>
          )}
