...
⚠️ **`queuedNotificationsCancelled` — не декоративное поле.** Режим проверки согласия на цель
`ProviderDelivery` — **`AccountsOnly`** (`ARCHITECTURE_CYCLE5.md` §52.3): отзыв этой цели
владельцем аккаунта **действительно прекращает уведомления** — уже поставленные в очередь
`Pending`-строки отменяются (их число и приходит в этом поле), новые не ставятся.

🔴 **Фронт обязан сказать об этом прямо** — и в предпросмотре, и в тексте подтверждения: отзыв цели
«передача привлекаемым лицам» означает, что сообщений о записи больше не будет. Это **не вольность
интерфейса, а исполнение опубликованного обещания**: `legal/04-pdn-consent.html`, цель 1, содержит
ровно эту фразу, и расхождение экрана с документом — самостоятельный дефект.

Отписка (`NotificationOptOut`, US-68 п. 6) при этом остаётся **отдельным** механизмом: она работает
по номеру телефона, действует и на гостя, и не отменяется возвратом согласия. Два пути к одному
результату — и человеку надо показывать тот, который он выбрал, не подменяя одно другим.
