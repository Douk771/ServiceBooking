...
> **Этот контур НЕ поддерживается как боевой** (решение Р4, SPEC цикла 3 §0.2, дата 2026-09-09).
> Боевой контур один — Linux + Docker + nginx, см. [`DEPLOY.md`](DEPLOY.md). Файлы этого раннбука
> **не удаляются** (могут пригодиться для демо-стенда), но в этом контуре **нет**: security-заголовков
> (US-47), автоматических бэкапов (US-40), деплоя с health-check и откатом (US-44) и наблюдаемости —
> логов/трекера ошибок (US-45). Для боевой эксплуатации он не годится. Если он когда-нибудь снова
> понадобится боевым — это отдельный цикл работ: всё перечисленное придётся делать заново под IIS.
...
(Какую ветку брать — см. модель веток в [`DEPLOY.md` §0.0](DEPLOY.md). Коротко: интеграционный
ствол — `develop`, там лежат все завершённые циклы; `master` получает код только через
`release-candidate` с тегом версии, и на момент написания этого абзаца релиза ещё не было, то есть
`master` отстаёт на десятки коммитов. **Не мёржьте циклы в `master` вручную ради деплоя** — для
демо-стенда берите `develop`.)
