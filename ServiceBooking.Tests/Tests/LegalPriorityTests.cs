...
public class LegalPriorityTests(TestDatabaseFixture fixture) : ApiTestBase(fixture)
{
    ...
    public async Task RevokeConsent_DeletesPhotosAndHealthNote_CancelsQueuedNotifications_PreviewMatchesActual()
    {
        ...
        using (var channelScope = Factory.Services.CreateScope())
        {
            ...
            await NotificationTestBase.EnsureWhatsAppPlanRuleAsync(db0, sub.PlanConfig.Id);
            var billingAccountId = await db0.Companies.Where(c => c.Id == company.Id).Select(c => c.BillingAccountId!.Value).FirstAsync();
            await NotificationTestBase.EnsureWhatsAppPaidAsync(db0, billingAccountId);
