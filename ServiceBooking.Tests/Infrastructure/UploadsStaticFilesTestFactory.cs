...
/// </summary>
public sealed class UploadsStaticFilesTestFactory(
    string connectionString, string? publicRootOverride = null, string? contentRootOverride = null)
    : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        // Same minimal settings every other factory seeds via TestHostSettings — see its own comment on
        // why UseSetting (not ConfigureAppConfiguration) is required for values Program.cs reads before
        ...
        if (contentRootOverride is not null)
        {
            ...
            // the repo's real manifest (not TestHostSettings' own temp copy) so this factory can still
            // register a user. This is orthogonal to what these tests actually pin (Storage:PublicRoot /
            // wwwroot resolution); it only exists to keep an unrelated dependency out of the way.
