...
**Актуально по состоянию на коммит: `6f39c96`, дата: 2026-09-23.**
...
|---|---|---|---|---|
| Юнит-тесты бэкенда | `ServiceBooking.UnitTests` | ничего | `dotnet test ServiceBooking.UnitTests` | 🔔 **1 077** по статическому счёту (было 942) |
| **Функциональные (API) тесты** | `ServiceBooking.Tests` | 🔬 **запущенный Docker** (не PostgreSQL!) | `dotnet test ServiceBooking.Tests` | 🔔 **620** по статическому счёту, 619 реальных (было 605) |
| Тесты фронтенда | `frontend/src/**/*.test.ts(x)` | Node 20 | `npm run test:run` (в `frontend/`) | 🔔 **419** тестов, 67 файлов (было 369 / 57) |
...
|---|---|---|---|
| Спека цикла 9, **редакция 2** | `SPEC.md` (корень) | markdown, §0–§7 | Корневой `SPEC.md` **занят циклом 9**. Редакция 2 переписала блок D: Web Push вместо «уведомлений только во вкладке», режим доставки выбирает владелец |
