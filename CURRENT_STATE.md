...
- 🔬 **`SPEC.md` в корне — это теперь спека ЦИКЛА 12** (подтверждение телефона через бота MAX).
  Спека цикла 8 архивирована в `SPEC_CYCLE8_TEST_ISOLATION.md`, спека цикла 11 — в
  `SPEC_CYCLE11_LEGAL_TEXTS.md` (сделано циклом 12 в первом же коммите). Ссылки вида «`SPEC.md` §…»
  из документов циклов 8 и 11 без правки ведут в чужую спеку — ровно ловушка §9 C5, теперь уже
  в третий раз. **Цикл 12 дополнительно кладёт копию своей спеки в `SPEC_CYCLE12_PHONE_VERIFICATION.md`
  в конце цикла**, не дожидаясь, пока её заархивирует следующий цикл: параллельно идёт
  `cycle/13-photo-logo-order-yandex-2gis`, и корневой `SPEC.md` — общий ресурс двух веток.
