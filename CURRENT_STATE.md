...
отсчитывайте от той точки `develop`, куда ляжет мёрж цикла 18**, а не от `91196aa`.

🎯 **Как читать эту редакцию (финальное закрытие цикла 18, «системный тариф Триал»).** Это
**точечное обновление** редакции `9864b3c` по диапазону `9864b3c..91196aa` — **69 коммитов на ветке
от `develop`, из них 33 после прошлой отметки** (четыре круга ревью, 32 файла, +7 442 / −88 в этом
хвосте); репозиторий заново не сканировался. Метка блоков цикла — 🎯.

🔴 **Первое, что нельзя прочитать неправильно (и это не изменилось): строки пробного тарифа в
каталоге нет, и цикл её не сидирует.** Сидов нет ни в `Program.cs`, ни в миграциях — есть только
частичный уникальный индекс. После выката пробный период **не работает вообще**, пока суперадмин
руками не создаст тариф, не отметит его пробным, не выставит лимиты и не расставит правила по
**каждой** опции каталога (опция без явного правила в триал не попадает, и добавленная в каталог
позже сама туда не попадёт) — §4.28 п. 0 и **C18-1**.

✅ **Второе: три части архитектуры, которых прошлая редакция не нашла в дереве, реализованы, и
найдена с тех пор четвёртая — тоже реализована.** Хук старта окна рассылок (§336), фоновая задача
`trial-lifecycle` (§337), правило уничтожения реестра номеров (§343) и **§333.3 — материализация
строки `AccountSubscriptionOption` при активации триала** (без неё рассылки на триале физически не
работали: резолвер берёт количество номеров только из этой строки, шлюз уведомлений блокирует при
нуле — при дословном обещании бесплатных рассылок в правовом тексте). `CHANGELOG.md`, `README.md` и
`API_DOCUMENTATION.md` с этого момента **не расходятся** с кодом в этой части — прежние пункты
**C18-2…C18-6** закрыты, см. §5.1-bis и §9.

🔴 **Третье: у цикла есть принятые остатки, и они названы остатками, а не «сделано»** — §5.1-bis
(новая редакция) и **C18-10…C18-13**: сужённая, но не устранённая гонка в фазе истечения;
продублированный в двух местах гейт «телефон подтверждён + состояние подсистемы подтверждения»;
отсутствие юнит-тестов на новую логику (`AppDbContext` в юнит-тестах — не конвенция этого проекта);
предусловие деплоя О15, которое из ветки не проверяется.

🔴 **Четвёртое, главный урок цикла и теперь конвенция (§6 🎯 п. 9): зелёный прогон не доказывает
наличия функционала.** Тест не может провалиться на коде, которого нет: четыре несущие части этого
цикла отсутствовали при **770 зелёных** функциональных тестах и четырёх кругах ревью. Проверка
«есть ли в дереве то, что документ объявляет работающим» делается **до** объявления готовности —
`grep` по именам заявленных классов и `ls` по каталогу задач/правил достаточно.

🔴 **Пятое, для следующего цикла, который тронет переход «триал → Free»: пять условий Т1–Т5 несут
правовую нагрузку, а не стилистику.** Правомерность схемы «предупреждаем только плашкой в кабинете»
держится в том числе на том, что на переходе **нет ни одного необратимого последствия**: превышение
лимитов — **заморозка** (нельзя добавлять новое), а не выключение компаний, и платежей на переходе
нет вовсе. Как только на переходе появится списание, удаление или реальное отключение компаний,
плашки станет недостаточно и продукту потребуется внеполосный канал связи — которого у платформы
нет. Читать **до** правки перехода: §4.28 «правовая рамка», **C18-8**, `LEGAL_REVIEW_CYCLE18.md` §5.

Что искать по метке 🎯: **§0.3** (что из §0.2 перестало быть правдой), **§3 блок 🎯** (две новые
сущности, 17 новых колонок, **три** миграции — файлов миграций стало **70**), **§4.28** (что цикл
сделал), **§5.0-nonies** (совместимость), **§5.1-bis** (принятые остатки с обоснованием), **§6 🎯**
(конвенции, включая п. 9), **§7 🎯** (наборы `CY18-`/`CY18L-`, числа прогона и две ловушки прогона),
**§8 🎯** (CI, переменные деплоя, предусловие О15), **§9 блок C18-1…C18-13** плюс ✅ на **C5** и
уточнение **C17-1**, §10.1/§10.3/§10.4/§10.5 (документы цикла).

Ни один блок 🧹/🔒/🗺/📱/🧾/📜/📸/🔔/🔬/💳/🗓/⚖️/🆕/🔁 не переписан и не сокращён; блоки §0.1 и §0.2
оставлены как снимок «до» и дополнены §0.3.

---

**Актуально по состоянию на коммит: `e9b7005` (`develop`, он же HEAD ветки `cycle/018-trial-plan` —
ветки посимвольно совпадают, `git log develop..HEAD` пуст), дата: 2026-09-25.**
Прошлая отметка была `9fa6947`. Отсчёт следующего diff — от `e9b7005`.

🧭 **Как читать эту редакцию (сверка на входе в цикл 18, «тариф Триал»).** Это **точечное
обновление-сверка**, а не новое сканирование. Диапазон `9fa6947..e9b7005` — **21 коммит, 75 файлов,
+9 481 / −294**, но **нового, ещё не описанного кода в нём нет**: содержательно это код цикла 16
(уже описан блоком 🔒), три мёрж-коммита (`f2d7c05` — цикл 16 в `develop`, `94452f8` — `develop` в
ветку цикла 17, `e9b7005` — цикл 17 в `develop`) и документы закрытия обоих циклов. **Новых
эндпоинтов — 0, новых сущностей и колонок — 0, миграций — 0** (в
`ServiceBooking.Infrastructure/Migrations/` по-прежнему **67** файлов миграций без `.Designer`).
Что добавлено этой правкой: настоящая шапка, §0.1 (снятие путаницы с двойной нумерацией §4.27 и
§5.0-octies) и §0.2 (сверенные по коду факты о тарифах — область, которую будет трогать цикл 18).
Ни один существующий блок не переписан и не сокращён.

### 0.1 Двойная нумерация §4.27 и §5.0-octies — это не ошибка чтения

Циклы 16 и 17 писались параллельно и друг о друге не знали, поэтому в документе **два разных
раздела `### 4.27`** (🧹 цикл 17 — отмена по правилам сервера, объяснения вместо молчания,
уведомления по снятому тарифу; 🔒 цикл 16 — гейт гостевых данных, сроки ответа субъекту, сигналы
оператору) и **два разных `### 5.0-octies`** (🧹 цикл 17 и 🔒 цикл 16). **На `e9b7005` в `develop`
влиты оба цикла**, поэтому оба блока описывают сегодняшнее состояние **одновременно** и оба
действительны. Предупреждение из редакции цикла 17 («цикл 16 о нём не знал, состояние стенда
смотрите в обеих») больше не про «что влито», а только про то, что тексты блоков друг на друга не
ссылаются.

### 0.2 Тарифы/подписки на входе в цикл 18 — факты, сверенные по коду (не по документам)

Сверка сделана чтением кода, потому что цикл 18 работает именно здесь. Разделы §3 «💳 Биллинг-
аккаунты и каталог тарифов», §4.6, §4.19 и §5.0-bis перечитаны и **расхождений с кодом не найдено**;
ниже — только то, чего в них не сказано явно, и что важно знать до начала работы.

1. **Понятия «триал»/«trial» в кодовой базе нет вообще.** `grep -rniI trial` по `*.cs`/`*.ts`/`*.tsx`
   в `ServiceBooking.API`, `ServiceBooking.Core`, `ServiceBooking.Infrastructure`, `frontend/src` —
   **ноль совпадений**. Нет ни поля, ни перечисления, ни колонки, ни признака «пробный период».
2. **Нет никакого механизма «одноразовости на биллинг-аккаунт».** В `BillingAccount`,
   `AccountSubscription`, `SubscriptionChangeLog` нет ни колонки «этим тарифом уже пользовались», ни
   уникального индекса такого смысла. Единственный след истории тарифов —
   append-only `SubscriptionChangeLogs` (`OldPlanConfigId`/`NewPlanConfigId`, `ChangeKind`), и он
   **никак не ограничивает** повторное назначение того же тарифа.
3. **Срок жизни подписки — одна колонка `AccountSubscriptions.PaidUntil`**, плюс `IsActive`. Даты
   «начала» у подписки в модели **нет** — только `CreatedAt`/`UpdatedAt` строки.
4. **У опции уже есть собственное окно действия, и оно уже проверяется.**
   `AccountSubscriptionOption.PaidUntilUtc` (null = «до конца периода подписки») и `EndsAtUtc`
   (выключение задним числом, строка остаётся историей). Проверка —
   `SubscriptionResolver.IsOptionCurrentlyPaid(subscriptionUsable, optionPaidUntilUtc,
   currentPlanAvailability, nowUtc)`: сначала обязательно **годна сама подписка** («опция не может
   жить дольше подписки, на которую куплена», N12), затем своя дата опции, затем правило текущего
   тарифа (`Extra`/`Included`; **отсутствие строки `PlanOptionRule` = `Unavailable`**, fail-closed).
   Пишет эти даты только `AdminBillingController` (строки ~570–593), владелец — никогда.
5. **Два флага видимости/доступности тарифа уже существуют и различаются по смыслу**:
   `SubscriptionPlanConfig.IsActive` («можно ли вообще назначить аккаунту»; деактивированный тариф
   роняет владельца во `EffectivePlan.Free`) и `IsPublic` («показывать ли в публичной витрине
   `GET /api/pricing`»). Третьего флага для этого заводить не нужно.
6. **`IsSystemFree` — enforcement в коде есть**, вопреки устаревшему комментарию в
   `ServiceBooking.Core/Entities/SubscriptionPlanConfig.cs` («enforced by the admin write endpoint,
   not implemented in this slice»): проверки живут в `AdminController` (`ValidateSystemFreeAsync`,
   строки ~535–660) и опираются на частичный уникальный индекс в `AppDbContext`; флаг меняется
   **отдельным маршрутом** `PUT /api/admin/plans/{id}/system-free`, в теле `AdminPlanInput` его нет.
   Комментарий в сущности читать как исторический.
7. **Где лежит админский CRUD тарифов** (это `AdminController`, а не `AdminBillingController`):
   `GET /api/admin/plans`, `POST /api/admin/plans`, `PUT /api/admin/plans/{id}`,
   `PUT /api/admin/plans/{id}/system-free`, `DELETE /api/admin/plans/{id}`. Назначение тарифа
   аккаунту — наоборот, в `AdminBillingController`:
   `PUT /api/admin/billing-accounts/{accountId}/subscription`. Старый
   `PUT /api/admin/owners/{ownerUserId}/subscription` отвечает **410 Gone** (§4.19).
8. **Возможности тарифа — это ровно поля `SubscriptionPlanConfig`** (см. §3): `MaxEmployees`,
   `MaxCompanies` (оба nullable, null = без ограничения), `AllowOnlineBooking`, `AllowMailing`,
   `AllowAnalytics`, `AllowPublicListing`, `AllowOnlinePayment`, `AllowNotificationChannel`,
   `PhotoQuotaMb`, `PhotoRetention`, `NotifyDaysBefore`, плюс витринные `Highlights`, `IsPublic`,
   `SortOrder`. Всё это разрешается в `EffectivePlan` (`SubscriptionResolver`), лимит сотрудников —
   **суммарный по аккаунту** плюс `BillingAccount.GrandfatheredEmployeeBonus`.
9. **Настройки платформы, включаемые из админки, живут в БД**, а не в `appsettings`:
   `PlatformSettings` + `PlatformSettingChangeLogs`, маршрут `GET|PUT /api/admin/platform-settings`
   (см. §4.19 про `pricing.public-enabled`). Это существующее место для «настраивается из админки».

🗄 **Архива `docs/history/` в проекте по-прежнему НЕТ, и эта правка его не заводила**: соглашения об
архиве в `README.md` нет (`grep docs/history README.md` — пусто), документы циклов лежат в корне
(`SPEC.md`, `ARCHITECTURE_CYCLE17.md`, `API_CONTRACT_CYCLE17.md` и т.д., см. §10.5). Документы цикла
17 **оставлены на месте**, чтобы не ломать десятки ссылок на них внутри этого документа.

### 0.3 🎯 Что из §0.2 перестало быть правдой после цикла 18

§0.2 выше — снимок «до», он намеренно не переписан. По коду на `91196aa` изменилось ровно это:

1. **п. 1 «понятия „триал“ в кодовой базе нет вообще» — неверно с цикла 18.** Появились
   `SubscriptionPlanConfig.IsSystemTrial`, 14 колонок `Trial*` на `BillingAccount`, две новые
   сущности (`TrialGrant`, `TrialPhoneRegistration`), перечисление `TrialGrantSource`, девять файлов
   `ServiceBooking.API/Services/Billing/Trial*.cs`, девять новых маршрутов API и шесть новых
   компонентов фронта — §3 блок 🎯 и §4.28.
2. **п. 2 «нет никакого механизма одноразовости на биллинг-аккаунт» — неверно.** Одноразовость
   держат **два частичных уникальных индекса БД**: `UX_TrialGrants_OnePerAccount` (фильтр
   `"Source" <> 2` — аварийная перевыдача исключена намеренно) и `UX_TrialPhoneRegistrations_Key`
   (реестр номеров, переживает удаление аккаунта). Проверки в `TrialActivationService` существуют
   ради понятного текста отказа, а не ради гарантии — гарантия на уровне БД.
3. **п. 3 «срок жизни подписки — одна колонка `PaidUntil`» — по-прежнему верно, и срок триала
   выражен именно ею** (своей колонки срока у триала нет, это требование заказчика R8). Добавилась
   **вторая дата другого смысла** — `AccountSubscriptions.MailingUntilUtc`: «рассылочные
   возможности ЭТОЙ подписки годны не дольше этой даты», правило общее для любого тарифа, и с
   реализацией хука §336 колонку **действительно заполняют** (`TrialMailingWindowStarter`). Даты
   «начала» у подписки так и не появилось — «когда начался триал» живёт на биллинг-аккаунте.
4. **п. 5 «двух флагов достаточно, третьего не нужно» — осталось верным для видимости и
   назначаемости**, но появился **системный флаг другого смысла**: `IsSystemTrial` (роль тарифа в
   каталоге, как `IsSystemFree`), со своим частичным уникальным индексом и своим отдельным
   маршрутом `PUT /api/admin/plans/{id}/system-trial`. В `AdminPlanInput` его нет — ровно по той же
   причине, что и `isSystemFree`.
5. **п. 6 «устаревший комментарий к `IsSystemFree`» — ✅ исправлен** в
   `ServiceBooking.Core/Entities/SubscriptionPlanConfig.cs`: теперь он называет, где живёт
   enforcement, вместо «not implemented in this slice». Это был отдельный пункт приёмки цикла.
6. **пп. 4, 7, 8, 9 — без изменений.** Окно опции и `IsOptionCurrentlyPaid` цикл не трогал;
   админский CRUD тарифов остался в `AdminController`, назначение аккаунту — в
   `AdminBillingController`; список возможностей тарифа цикл не расширял (триал набирается теми же
   полями и правилами опций); настройки пробного периода легли **в `PlatformSettings`** (БД), как и
   предполагал п. 9, — три новых ключа, см. §4.28.
7. **🎯 Добавилось после прошлой редакции (четыре круга ревью):** у `AccountSubscriptionOption`
   появился флаг **`GrantedByTrial`** (третья миграция цикла) — им отличается строка опции,
   материализованная самим триалом, от оплаченной админом; фоновых задач стало **восемь**
   (`trial-lifecycle`), правил уничтожения — **девятнадцать** (`TrialPhoneRegistrationRule`), и в
   `GET /api/admin/retention/policy` дополнительно попали **три** срока: `TrialPhoneRegistrationDays`
   и — попутным закрытием долга цикла 14 — `PhoneVerificationSessionDays`/`VerifiedPhoneOrphanDays`.
...
осознанное решение заказчика (`SPEC_CYCLE15_BOOKING_CARD_TARIFF.md` §0-bis П1), а не побочный эффект: **у всех компаний,
...
### 🎯 Системный тариф «Триал»: две новые сущности и 17 новых колонок (цикл 18)

Всё аддитивно: ни одного `NOT NULL` на существующей таблице, ни одного переименования, ни одной
миграции данных.

**Новые колонки на существующих таблицах (17):**
* `SubscriptionPlanConfigs.IsSystemTrial` — `bool NOT NULL DEFAULT false`, **второй системный флаг**
  каталога. Частичный уникальный индекс в `AppDbContext`
  (`HasFilter("\"IsSystemTrial\" = true")`) — строка каталога с этим флагом может быть только одна.
  🔴 **Строки с этим флагом в БД нет: сидов нет ни в `Program.cs`, ни в миграциях** (см. §4.28 п. 0).
* `AccountSubscriptions.MailingUntilUtc` — `timestamptz NULL`, «рассылочные возможности этой
  подписки годны не дольше этой даты», `null` = «до конца самой подписки». Названа **не**
  `TrialMailingUntil` намеренно: это свойство подписки, а не признак триала. 🎯 **Заполняется**
  `TrialMailingWindowStarter` при первой авторизации канала (§336) и фазой 1 задачи
  `trial-lifecycle` (самолечение пропущенного старта).
* `BillingAccounts` — **14 nullable-колонок** состояния: `TrialStartedAtUtc`, `TrialEndsAtUtc`,
  `TrialDurationDays`, `TrialMailingWindowDays`, `TrialWarningThresholdsDays` (`string(32)`,
  снапшот «7,3,1»), `TrialTermsVersion` (`string(32)`), `TrialTermsAcknowledgedAtUtc` (set-once),
  `TrialGrantSource`, `TrialGrantedByUserId`, `TrialChannelFirstAuthorizedAtUtc`,
  `TrialMailingWindowEndsAtUtc`, `TrialExpiredHandledAtUtc`, `TrialWarnedAtThresholdDays`,
  `TrialMailingClosureLoggedAtUtc`. 🎯 **Все 14 действительно заполняются**: последние пять —
  хуком §336 (`TrialChannelFirstAuthorizedAtUtc`, `TrialMailingWindowEndsAtUtc`) и фазами задачи
  `trial-lifecycle` (`TrialExpiredHandledAtUtc`, `TrialWarnedAtThresholdDays`,
  `TrialMailingClosureLoggedAtUtc`). `TrialStartedAtUtc` — отметка «этим аккаунтом триал
  использован», она не стирается ни истечением, ни сменой тарифа.
* 🎯 **`AccountSubscriptionOptions.GrantedByTrial`** — `bool NOT NULL DEFAULT false`, **третья
  миграция цикла**. Смысл: «эту строку опции материализовал сам триал» (§333.3). Им отличается
  триальная строка от оплаченной админом — активация чужую строку не перезаписывает, истечение гасит
  только свои, а `AssignSubscription` флаг **всегда сбрасывает в `false`** (иначе аварийная
  повторная выдача молча переписала бы оплаченную строку — её количество и срок).
* Индекс `IX_BillingAccounts_TrialExpiry` (фильтр `TrialEndsAtUtc IS NOT NULL AND
  TrialExpiredHandledAtUtc IS NULL`) — единственная выборка фазы 4 задачи `trial-lifecycle`;
  условие намеренно «дата прошла», а не «дата = сегодня», чтобы пропущенный проход догонялся.

**`TrialGrant` (таблица `TrialGrants`) — append-only журнал выдач и доказательство Т1.** Поля:
`Id`, `BillingAccountId`, `PlanConfigId?`, `GrantedAtUtc`, `EndsAtUtc`, `DurationDays`,
`MailingWindowDays`, `WarningThresholdsDays` (`string(32)`), `Source`, `GrantedByUserId`, `Reason?`
(`string(500)`, обязателен только у аварийной перевыдачи) плюс доказательная пятёрка Т1:
`TermsVersion` (`string(32)`, NOT NULL), `TermsTextSha256` (`string(64)`, NOT NULL — хеш **шаблона**
текста, не подставленной строки), `TermsShownAtUtc?`, `TermsAcknowledgedAtUtc?` (оба **set-once**).
Индексы: `UX_TrialGrants_OnePerAccount` — уникальный по `BillingAccountId` с фильтром
`"Source" <> 2` (**это и есть гарантия «один триал на аккаунт»**, и она же ловит гонку двойного
клика; аварийная перевыдача из фильтра исключена намеренно) и обычный по `GrantedAtUtc`.
Строки не входят ни в одно правило уничтожения — это доказательство исполнения обязанности сообщить
условия, номера и его производных в них нет.

**`TrialPhoneRegistration` (таблица `TrialPhoneRegistrations`) — реестр предоставленных пробных
периодов, живёт дольше аккаунта.** Поля: `Id`, `PhoneKeyHash` (`string(64)`, lowercase hex,
HMAC-SHA256 от нормализованного номера с префиксом `trial-phone:`), `RegisteredAtUtc` (дата
предоставления — от неё считаются три года), `KeyId` (`string(16)`, NOT NULL — на каком ключе
вычислено значение). Индексы: уникальный `UX_TrialPhoneRegistrations_Key`, плюс по `RegisteredAtUtc`
и по `KeyId` (под правило уничтожения `TrialPhoneRegistrationRule` — 🎯 оно реализовано, см. §4.28). 🔴 **Ни `FK`, ни `UserId`, ни
`BillingAccountId`, ни email здесь нет и появляться не должны** — ссылка на аккаунт вернула бы
каскад и убила бы смысл таблицы (иначе «удалил аккаунт → зарегистрировался заново → второй триал»).
Значение ни в одном DTO не отдаётся. 🔴 По правовой квалификации цикла это **не обезличенные
данные**, а полноценные ПДн со всеми обязанностями оператора.

**Перечисления.** `TrialGrantSource` (`OwnerSelfService = 0`, `SuperAdmin = 1`,
`SuperAdminOverride = 2`) — **append-only**: значение `2` участвует в сыром SQL-фильтре индекса.
`SubscriptionChangeKind` дополнен `TrialGranted = 6`, `TrialExpired = 7`, `TrialMailingWindow = 8`;
🎯 **пишутся все три**: 6 — активацией, 7 и 8 — фазами задачи `trial-lifecycle` (журнал append-only,
и при упавшей итерации строка журнала в базе **не остаётся** — это была находка третьего ревью).

🎯 **Три миграции**, все добавочные:
`20260925145043_AddTrialPlanFlagAndAccountTrialState` (флаг тарифа, `MailingUntilUtc`, 14 колонок,
два индекса), `20260925145131_AddTrialGrantAndPhoneRegistry` (две таблицы с их индексами) и
`20260925200311_AddGrantedByTrialToAccountSubscriptionOption` (одна колонка, добавлена на четвёртом
круге ревью вместе с §333.3). Снапшот (`AppDbContextModelSnapshot.cs`) закоммичен теми же
изменениями; файлов миграций без `.Designer` и без снапшота стало **67** (было 64), всего файлов в
каталоге без `.Designer` — **70**. Проверка непрерывности снапшотов — **68 проверок**, потерь
свойств нет.
...
🎯 **Цикл 18 — на ветке `cycle/018-trial-plan` файлов миграций без `.Designer` стало 69** (две
добавочные миграции цикла, см. блок 🎯 выше). Обе созданы штатным `dotnet ef migrations add`, разрыва
снапшота нет; проверка непрерывности снапшотов в CI (шаг цикла 17) проходит.
...
7. 🔒 `subject-request-due-soon` — см. блок 🔒 цикла 16.
8. 🎯 **`trial-lifecycle`** (период — **1 час**, цикл 18) —
   `Services/Scheduling/Tasks/TrialLifecycleTask.cs` (581 строка), **восьмая** задача в дереве.
   Четыре фазы за проход: самолечение пропущенного старта окна рассылок → закрытие просроченных окон →
   пороговые предупреждения «за 7, 3 и 1 день» → материализация перехода «триал → Free». В отличие от
   `DataRetentionTask` (дефект N9-6) она **изолирует ошибки и по фазе, и по отдельному аккаунту** и
   ходит keyset-курсором во всех фазах; сбой фазы виден в `ScheduledTaskOutcome.Error`. Подробно — §4.28.
   ⚠️ Пункты нумерации выше («задач теперь четыре») — снимок цикла 5; считайте по файлам в
   `Services/Scheduling/Tasks/`, их **восемь**.
...
### 4.28 🎯 Системный тариф «Триал» — функция цикла 18, рубильника нет, но каталог пуст

**0. 🔴 Главное предусловие: строки тарифа-триала в каталоге нет, и цикл её не сидирует.** Ни
`Program.cs`, ни миграции её не создают; в БД есть только частичный уникальный индекс, который
гарантирует, что таких строк не станет две. Пока строки нет, `GET /api/billing/trial` отвечает
`Unavailable`/`TrialNotOffered`, а `POST` — `409 TrialNotOffered`. Чтобы триал заработал, суперадмин
обязан: создать тариф (`POST /api/admin/plans`, цена **0**), отметить его пробным
(`PUT /api/admin/plans/{id}/system-trial`), выставить лимиты и **расставить правила по каждой опции
...
* владелец сам — `POST /api/billing/trial` (`[Authorize]`, политика rate limit `trial-activate`,
  5/сутки на пользователя, `RateLimits:TrialActivatePerDay`). Тело несёт `termsVersion` — редакцию
  условий, которую клиент только что показал человеку; несовпадение с текущей = `409
  TrialTermsVersionMismatch` и отказ. Аккаунта эндпоинт **не создаёт**: нет биллинг-аккаунта → `404`;
* суперадмин — `POST /api/admin/billing-accounts/{accountId}/trial` (те же проверки) и
  `POST /api/admin/billing-accounts/{accountId}/trial/regrant` — **аварийная перевыдача**, обходит
  отметку «уже пользовался» и требование подтверждённого телефона, но **обязательна причина текстом**
  и запись остаётся в `TrialGrants` с автором. Проверку однократности по номеру перевыдача **не**
  обходит.

**Порядок отказов фиксирован и является частью контракта** (форма запроса → состояние платформы →
собственное прошлое аккаунта → чужое прошлое): `TrialTermsVersionMismatch`, `TrialNotOffered`,
`TrialAlreadyActive`, `AlreadyOnPaidPlan`, `TrialAlreadyUsed`, `PhoneNotVerified`,
`TrialUniquenessCheckUnavailable`, `TrialPhoneAlreadyUsed`. 🎯 Девятый код —
`PhoneVerificationUnavailable` — **достижим** (требование R7): его отдают и активация, и чтение
состояния, когда подсистема подтверждения телефона выключена/недоступна, а не когда владелец просто
не подтвердил номер (тогда прежний `PhoneNotVerified`). ⚠️ Этот гейт **продублирован в двух местах**
— в `TrialActivationService` и в `TrialStateReader`; оба читают один источник, но структурной
гарантии, что они не разойдутся, нет (§5.1-bis п. 2). Текст отказа по
чужому номеру не несёт ни даты, ни имени, ни намёка на существование другого аккаунта — «ответ не
оракул» (конвенция цикла 16).

**Успешная выдача — одна транзакция под advisory-lock по биллинг-аккаунту:** подписка переводится на
тариф-триал (`IsActive = true`, `PaidUntil = now + N дней`, `MailingUntilUtc = null`), на аккаунт
пишутся снапшоты (длительность, окно рассылок, пороги «7,3,1», версия условий, источник и автор),
добавляется строка `TrialGrants` с доказательством Т1, строка `TrialPhoneRegistrations` (если
проверка включена и номер подтверждён) и запись `SubscriptionChangeLog` с `ChangeKind =
TrialGranted`. Гонку двух активаций ловит уникальный индекс: проигравший получает тот же честный
отказ, что дал бы последовательный второй запрос, **с датой победителя**, а не с «now».

**Проверка однократности — fail-closed, в двух уровнях.** `TrialPhoneKey` считает HMAC-SHA256 на
**собственном** ключе `Trial:PhoneKeyHmac` (свой `KeyId`). Если проверка включена, а ключ пуст / не
base64 / короче 32 байт — активация отказывает кодом `TrialUniquenessCheckUnavailable`, но
**никогда** не выдаёт без проверки. Вне Development `DeploymentSafetyChecks.ValidateTrialSecrets`
**роняет старт приложения**, если проверка выключена, ключ непригоден или совпадает с
`PhoneVerification:ExternalKeyHmac` либо `Notifications:EncryptionKey` (требование К2 — разделение
ключей между несовместимыми целями).

**Доказуемость условий (Т1).** `TrialTermsRegistry` держит текущую редакцию `2026-09-26`, её
SHA-256 считается по **шаблону** (до подстановки даты/названия), архив редакций — словарь (сегодня в
нём одна запись). Подтверждение — set-once: `POST /api/billing/trial/terms-acknowledgement`
(нужен для выдачи суперадмином — владелец текста ещё не видел), повторный вызов идемпотентен и
**не перештампует** дату. Архивную редакцию по версии отдаёт `GET /api/admin/trial-terms/{version}`.

**Горячий путь: `SubscriptionResolver` о триале не знает** (требование заказчика R8 —
`grep -n "IsSystemTrial" ServiceBooking.API/Services/SubscriptionResolver.cs` пуст). Срок триала —
существующая `PaidUntil`, поэтому истечение доступа работает само: подписка перестаёт быть годной и
владелец падает во `EffectivePlan.Free`. Единственное добавленное правило — **общее, не про триал**:
если `sub.MailingUntilUtc` в прошлом, у эффективного тарифа гасятся `AllowMailing` и
`AllowNotificationChannel`. 🎯 Правило **рабочее**: колонку заполняет `TrialMailingWindowStarter`.

🎯 **Рассылки на триале физически работают только вместе с §333.3 — это нашли на четвёртом круге
ревью.** Резолвер берёт количество оплаченных номеров **только** из строки
`AccountSubscriptionOption`, а шлюз уведомлений блокирует отправку при нуле — то есть без такой
строки владелец на триале видел обещанные бесплатные рассылки в правовом тексте и не мог отправить
ни одной. Поэтому активация **материализует** строку опции `notifications.whatsapp`
(`Quantity` из `PlanOptionRule.IncludedQuantity`, `PaidUntilUtc = TrialMailingWindowEndsAtUtc`,
`GrantedByTrial = true`) — но только если правило опции на триальном тарифе стоит `Included`
(fail-closed, конвенция цикла 7). Отсутствие правила/опции в каталоге — это не тихий отказ:
пишется `LogError` и уходит сигнал в GlitchTip, **вне транзакции и вне взятой блокировки**. Чужую
строку (`GrantedByTrial = false`, например оплаченную админом) активация **не трогает**.

🎯 **Старт окна рассылок — хук на первую авторизацию канала (§336).**
`ServiceBooking.API/Services/Billing/TrialMailingWindowStarter.cs` вызывается из
`ChannelStateTransition` (и через контроллеры каналов/уведомлений): при первом переходе канала в
«подключён» у аккаунта на живом триале ставятся `TrialChannelFirstAuthorizedAtUtc`,
`TrialMailingWindowEndsAtUtc` и `AccountSubscriptions.MailingUntilUtc`, а `PaidUntilUtc` триальной
строки опции подтягивается к концу окна. Пропущенный старт догоняет **фаза 1** задачи
`trial-lifecycle` («самолечение»), датируя окно **настоящей** самой ранней авторизацией. После
аварийной повторной выдачи колонки окна сбрасываются, и дата «окно открыто» не может оказаться
раньше начала текущего триала (иначе окно открывалось бы задним числом и сразу мёртвым).

🎯 **Фоновая задача `trial-lifecycle` — восьмая задача планировщика** (период 1 час,
`Services/Scheduling/Tasks/TrialLifecycleTask.cs`, 581 строка; регистрируется в `Program.cs`, актор
`system:trial-lifecycle`). Четыре фазы за проход: самолечение старта окна → закрытие просроченных
окон (журнал `TrialMailingWindow`) → пороговые предупреждения «за 7, 3 и 1 день» (снапшот порогов со
строки аккаунта) → материализация перехода «триал → Free» с записью `TrialExpired`. Свойства,
доведённые ревью: **изоляция ошибок и по фазе, и по отдельному аккаунту** (упавший аккаунт не
роняет ни пачку, ни последующие фазы; его незакоммиченные правки отбрасываются), **keyset-курсоры во
всех фазах** (иначе пачка постоянно падающих аккаунтов крутится до конца бюджета времени),
**fail-closed** при отсутствии тарифа с `IsSystemFree` (аккаунт не переносится никуда, ошибка видна
в сводке задачи и уходит сигналом оператору), и сбой фазы **виден** в `ScheduledTaskOutcome.Error`, а
не только в логах. Фаза истечения гасит **только** строки опций с `GrantedByTrial = true`.

🎯 **Уничтожение реестра номеров — 19-е правило retention**
(`Services/Retention/Rules/TrialPhoneRegistrationRule.cs`). Две причины удаления строки: возраст с `RegisteredAtUtc` больше
`Retention:TrialPhoneRegistrationDays` (по умолчанию **1095** дней = обещанные три года) и К3 —
строка вычислена на ключе, который больше не текущий (`Trial:PhoneKeyId`). ⚠️ Отдельная тонкость,
из-за которой правило переписывали: **пустая строка в настройке ключа не считается настроенным
ключом** — иначе К3-условие вычистило бы весь реестр однократности целиком. Удаления по ротации
отчитываются отдельным счётчиком, а не смешиваются со «сроком истёк».

🎯 **Сам пробный тариф нельзя назначить обычным админским маршрутом.**
`POST /api/admin/billing-accounts/{id}/subscription` (`AssignSubscription`) на тарифе с
`IsSystemTrial` отвечает **`409 TrialPlanNotAssignableHere`**: выдача триала идёт только через
`…/trial` и `…/trial/regrant`, которые пишут доказательство Т1 и реестр номеров. Тот же маршрут
всегда сбрасывает `GrantedByTrial` в `false` — оплаченная строка опции перестаёт выглядеть триальной.

🎯 **`GET /api/admin/retention/policy`** отдаёт `TrialPhoneRegistrationDays` и — попутным закрытием
долга цикла 14 — `PhoneVerificationSessionDays` и `VerifiedPhoneOrphanDays`, которые в этот ответ
никогда не были проведены.

**Экран владельца «Ваша подписка».** `GET /api/billing/trial` отдаёт `TrialStateDto` c состоянием
`Available` / `Unavailable` / `Active` / `Expired` (плюс объект `mailingWindow`, который в ответе
всегда присутствует, `Never`/`NotApplicable` вместо `null`); тот же объект приезжает полем `trial`
в `GET /api/billing/subscription` и в ответе `POST /billing/subscription/request`, чтобы экран не
расходился сам с собой. Состояние `Unavailable` называет причину теми же кодами и текстами, что дал
бы реальный `POST` (сухой прогон тех же проверок). Компоненты:
`components/billing/TrialCard.tsx` (карточка, не закрывается, дата видна всегда — Т2),
`TrialActivationTerms.tsx` (условия целиком + подтверждение — Т1/Т5),
`TrialExpiredNotice.tsx` (сообщение о состоявшемся переходе, закрыть нельзя, живёт не меньше 30 дней
— Т3), `TrialBanner.tsx` (закрываемое напоминание о пороге — 🎯 **достижим**, пороги отдаёт фаза 3
задачи `trial-lifecycle`; после продления срока триала устаревшая плашка **гаснет**),
`utils/trialError.ts` (14-й маппер кодов отказа). 🎯 Все места ответа читают конец триала из **одной**
колонки `TrialEndsAtUtc` — расхождение двух разных дат конца в одном ответе API было найдено ревью и
устранено.

**Заморозка вместо выключения (Д3).** `SubscriptionUsageDto` получил `overLimitCompanies`,
`overLimitEmployees`, `overLimitText`; поля наполняются **только когда аккаунт действительно
скатился на системный Free** (проверка зеркалит резолвер — иначе текст «Бесплатный тариф
допускает…» описывал бы чужое состояние), и текст цитирует лимиты именно системного Free. Ничего не
удаляется и не отключается — нельзя только добавлять новое.

**Витрина и админка.** `PublicPlanDto` получил `isTrial` — бейдж печатают и публичная витрина
`GET /api/pricing`, и предпросмотр `GET /api/admin/pricing/preview`
(`components/pricing/PlanCard.tsx`). Каталог тарифов защищён симметрично: триалом нельзя отметить
тариф с ценой > 0 (`400`), нельзя отметить второй тариф (`409`), нельзя пометить триал системным
бесплатным и наоборот (`409` в обе стороны), нельзя удалить триал (`409`), нельзя деактивировать при
живых подписчиках (`409`). Настройки пробного периода — **три ключа `PlatformSettings`**
(`trial.duration-days`, `trial.mailing-window-days`, `trial.warning-thresholds-days`, дефолты 14 / 7
/ «7,3,1») с журналом изменений; `PUT /api/admin/platform-settings` отклоняет диапазоны вне
`1..365`, окно больше длительности, пороги не 1..5 различных положительных значений и — отдельно —
**пороги, разошедшиеся с тем, что буквально обещает текущая редакция текста условий**. Правка
настроек действует только на новые выдачи: уже выданный триал читает свой снапшот (Д19).
`GET /api/admin/billing-accounts` получил фильтр `?trial=`, карточка аккаунта — блок «Пробный
период» (состояние, даты, источник, редакция условий и когда подтверждена, окно рассылок, все
выдачи с причинами). Экраны: `pages/admin/PlansTab.tsx`, `BillingAccountsAdminTab.tsx`,
`NotificationsAdminTab.tsx`.

**Удаление аккаунта.** `GET /api/profile/delete-account/preview` (новый) отдаёт
`trialRegistryNotice` — предупреждение, что отметка в реестре переживёт удаление; непустой только у
того, кому триал выдавался. Маршрут внесён в allow-list `LegalConsentFilter` рядом с самим
удалением: право уйти из сервиса не гейтится принятием новой редакции документов. Экран —
`components/profile/TrialRegistryDeletionNotice.tsx` на `/profile/delete-account`.

**Правовая рамка — читать до любой правки этой области.** Все тексты написал юрист **до** кодирования
и лежат отдельным файлом `ServiceBooking.API/Services/Billing/TrialLegalNotices.cs` (20 констант;
`LegalNotices.cs` цикл не трогал вовсе). Правомерность схемы «единственный канал оповещения —
кабинет» признана **при пяти условиях Т1–Т5**: (Т1) факт показа условий фиксируется с версией
текста; (Т2) дата окончания видна постоянно, а не только в закрываемой плашке; (Т3) сообщение о
состоявшемся переходе живёт не меньше 30 дней и не закрывается; (Т4) **на переходе нет ни одного
необратимого последствия** — ни списаний, ни удаления, ни фактического выключения компаний и
сотрудников; (Т5) текст активации честно называет, что другого способа сообщить у платформы нет.
🔴 **Т4 — несущая конструкция, а не вежливость:** появление на переходе списания, удаления или
реального отключения компаний делает плашку в кабинете недостаточной и требует внеполосного канала
связи, которого в продукте нет. ⚠️ Отдельно: **в тексте условий пробного периода почта намеренно НЕ
обещана** (п. 6.16.11 Соглашения), тогда как пп. 4.1 «б», 6.13.8 и 16.2 того же Соглашения e-mail
обещают — это долг комплекта, цикл его не создавал и не закрывал (**C18-8**).
Правовой комплект правился: п. 1.1.25, **6.16** (целиком про пробный период) и 6.13.17 в
`03-terms-owner.html`, п. **5.6.7** плюс вставки в 12.4/13.2 в `01-privacy-policy.html`; редакции
Политики и Соглашения с владельцем подняты до `2026-09-26-draft`, `isDraft` остался **true**,
артефакт `ServiceBooking.API/App_Data/legal/` пересобран, `ServiceBooking.LegalKit check` проходит.

**Рубильника у функции нет** — она включается наличием строки тарифа в каталоге (см. п. 0).
🎯 Что цикл **сознательно оставил остатком** (а не сделал) — §5.1-bis в новой редакции и
**C18-10…C18-13**.
...
заполнены, блока переходов на карточке нет вовсе. Это решение заказчика (`SPEC_CYCLE15_BOOKING_CARD_TARIFF.md` §0-bis П1),
...
- 🧹 **`SPEC.md` в корне — это спека ЦИКЛА 17** (закрытие техдолга C15; дата 2026-09-25, ветка
  ...
  ✅ **Снято циклом 18 (задача D1, раздел 7 спеки цикла 18).** Спеки циклов 13, 15 и 17 положены в
  дерево как `SPEC_CYCLE13_PHOTO_LOGO_MAPS.md`, `SPEC_CYCLE15_BOOKING_CARD_TARIFF.md` и
  `SPEC_CYCLE17_C15_TECHDEBT.md` (восстановлены из `4058ae9`, `f45fe56` и `e9b7005`), а ссылки
  «`SPEC.md` §…» из документов этих циклов перенаправлены на архивные имена в том же коммите —
  см. §10.5. Корневой `SPEC.md` теперь означает **только спеку текущего цикла 18**.
...
🎯 **Дополнение цикла 18 к §5.4 — ✅ расхождение снято, но знать историю полезно.** Запись цикла 18 в
`CHANGELOG.md`, абзацы `README.md` («пробный период» и «чего пока нет») и раздел
`API_DOCUMENTATION.md` были написаны по архитектуре цикла и утверждали как работающее четыре
механизма, которых тогда в коде не было: предупреждения за 7, 3 и 1 день; окно рассылок, которое
стартует с привязкой первого номера и закрывается через 7 дней; автоматический переход «триал → Free»
фоновым проходом с догоном пропущенных суток; хранение реестра номеров три года. **Все четыре
реализованы в этом же цикле (§4.28), тексты править не потребовалось** — документы стали правдой, а
не наоборот. README **честен** и про два оставшихся факта — что строки тарифа в каталоге нет и что
единственный канал оповещения — кабинет. ⚠️ Развёрнутая пользовательская документация (`docs/`) про
пробный период по-прежнему не знает вообще — §10.2.

🔴 **Урок, который стоит держать при чтении любого раздела этого файла:** расхождение «документ
свежий и подробный, кода нет» цикл 18 прожил в самой дорогой форме — оно было незаметно при 770
зелёных тестах и трёх кругах ревью. Сверяйте заявленное `grep`'ом по именам классов (§6 🎯 п. 9).
...
### 5.0-nonies 🎯 Совместимость после цикла 18 — для API ничего не сломано, для выката сломано

**Для звавших API по-старому — ни одного ломающего изменения.** Все новые поля добавлены с
значениями по умолчанию и в конец позиционных `record`-ов: `OwnerSubscriptionDto.Trial = null`,
`SubscriptionUsageDto.OverLimitCompanies/OverLimitEmployees/OverLimitText`, `PublicPlanDto.IsTrial =
false`, `AdminPlanDto.IsSystemTrial`/`OptionCoverage`, `AdminPlatformSettingsDto` + три поля. Новых
обязательных полей в запросах нет; `PUT /api/admin/platform-settings` трактует отсутствующее поле
как «не менять».

**Одно поведенческое изменение на общем пути, и оно 🎯 рабочее.** `SubscriptionResolver.Resolve`
гасит `AllowMailing` и `AllowNotificationChannel`, если `AccountSubscriptions.MailingUntilUtc` в
прошлом. Правило общее для любого тарифа; колонку заполняет `TrialMailingWindowStarter`, поэтому для
аккаунта на триале рассылки действительно выключаются по окончании окна, а не по окончании триала.

🎯 **Второе поведенческое изменение — на админском маршруте назначения тарифа.**
`POST /api/admin/billing-accounts/{id}/subscription` отвечает **`409 TrialPlanNotAssignableHere`**,
если указан тариф с `IsSystemTrial` (раньше назначил бы его как любой другой). Тот же маршрут теперь
всегда сбрасывает `AccountSubscriptionOption.GrantedByTrial` в `false`. Для всех остальных тарифов
поведение дословно прежнее.

🔴 **Для выката ломающее изменение есть, и оно про конфигурацию.** Вне `Development` приложение
**не стартует**, если `Trial:UniquenessCheck:Enabled` выключен, либо `Trial:PhoneKeyHmac` /
`Trial:PhoneKeyId` непригодны, либо ключ совпадает с `PhoneVerification:ExternalKeyHmac` или
`Notifications:EncryptionKey` (`DeploymentSafetyChecks.ValidateTrialSecrets`). То есть выкат этой
ветки на боевую машину **без новых переменных окружения падает на старте** — §8 🎯.

### 5.1-bis 🎯 Принятые остатки цикла 18 — что названо остатком, а не «сделано»

**Прошлая редакция этого раздела перечисляла четыре части архитектуры, которых не было в дереве
(старт окна рассылок §336, задача `trial-lifecycle` §337, правило уничтожения реестра §343 и
недостижимый код отказа `PhoneVerificationUnavailable`). Все они реализованы; пятой, не названной
тогда нигде — ни здесь, ни в каталоге тестов — оказалась §333.3 (материализация строки
`AccountSubscriptionOption`), она тоже реализована.** Раздел заменён на честный список того, что
цикл **осознанно** оставил как есть. Каждый пункт — с обоснованием и ценой решения.

1. **Гонка в фазе истечения СУЖЕНА, но не устранена.** Перед мутацией фаза 4 `TrialLifecycleTask`
   делает **свежее чтение тарифа именно этого аккаунта** из БД, а не доверяет снапшоту всей пачки.
   Окно, в котором решение админа/владельца (`AssignSubscription` на платный тариф) может быть молча
   переписано обратно во Free, сократилось с «время обработки всей пачки» до «между этим чтением и
   `SaveChangesAsync` той же итерации» — с секунд до одного обращения к базе. **Полное закрытие
   требует advisory-lock, взятого до обоих действий, и перечитывания под ним**; это меняет структуру
   батчевой оптимизации, и это **не делалось сознательно** — на последнем круге ревью перестройка
   пер-аккаунтных транзакций/блокировок рисковала и оптимизацией, и покрытием `CY18L-19/24`. Цена
   уже принятого сужения тоже названа: **примерно удвоение числа обращений к базе за проход** на
   большом бэклоге. **C18-10**.
2. **Гейт «телефон подтверждён + состояние подсистемы подтверждения» продублирован в двух местах** —
...
3. **Юнит-тестами новая логика покрыта частично.** `TrialActivationService` и `TrialLifecycleTask`
   ...
   Заводить параллельный стиль юнит-тестов в разрез с конвенцией не стали; из юнит-набора цикл
   дополнил только `DeploymentSafetyChecksTests`. Следствие: чистые функции (`TrialWindow`,
   `TrialPhoneKey`) покрыты юнитами, а сервис и задача — только функциональными. **C18-12**.
4. **Строки пробного тарифа в каталоге по-прежнему нет, и цикл её не сидирует** — §4.28 п. 0 и
   **C18-1**. Это не изменилось ни за один из четырёх кругов ревью: триал **не работает сразу после
   деплоя**, пока суперадмин не создаст тариф, не отметит его пробным, не выставит лимиты и не
   расставит правила по каждой опции. Опция без явного правила в триал не попадает, добавленная
   позже — тоже.
5. **Предусловие деплоя О15 из ветки не проверяется** — **C18-9** и §8 🎯: перед выкатом нужно на
   **живой** базе убедиться, что опубликованный правовой комплект не активен и в `ConsentRecord` нет
   записей на предыдущие черновые редакции.
6. **Чужой долг, который цикл не расширял и не закрывал** — **C18-8**: правки цикла 16 в правовой
   комплект так и не внесены, а Соглашение с владельцем **в трёх пунктах** (4.1 «б», 6.13.8, 16.2)
   обещает уведомления по e-mail, которого в продукте нет вовсе. Это **опаснее** ситуации с триалом,
   потому что касается цены и условий договора, тогда как по пробному периоду почта намеренно **не**
   обещана (п. 6.16.11).
...
### 🎯 Конвенции, введённые циклом 18 — им следовать, а не заводить рядом своё

1. 🔴 **Подсистема с fail-closed по секрету обязана получить тестовый секрет в
   `ServiceBooking.Tests/Infrastructure/CustomWebApplicationFactory.cs`.** Это не косметика:
   `appsettings.json` намеренно везёт пустой `Trial:PhoneKeyHmac` (ключ не должен жить в
   репозитории), `Trial:UniquenessCheck:Enabled` по умолчанию `true`, и до появления двух строк
   `builder.UseSetting("Trial:PhoneKeyHmac"/"Trial:PhoneKeyId", …)` **любая** активация триала в
   **любом** тестовом хосте отказывала кодом `TrialUniquenessCheckUnavailable`. Вся активация была
   непокрываема тестами, и это не было видно: тесты, которых нет, зелёные. Конвенция уже
   существовала для `Notifications:EncryptionKey` — новый цикл обязан её повторить для своего
   секрета, фиксированным детерминированным значением.
2. **Системный флаг тарифа меняется только своим отдельным маршрутом** (`…/system-free`,
   `…/system-trial`) и в `AdminPlanInput` не появляется — обычная правка полей не должна иметь
   возможности случайно его переставить. Проверки взаимоисключения ставятся **симметрично, в обе
   стороны**: цикл нашёл, что `SetSystemFree` не смотрел на `IsSystemTrial`, и на пустой базе тариф
   мог получить оба флага.
3. **Обещанные человеку числа хранятся снапшотом на строке, а живая настройка действует только на
   новые выдачи** (Д19): длительность, окно рассылок и пороги предупреждений скопированы в
   `BillingAccount.Trial*` и в `TrialGrants` на момент выдачи. Тот же приём уже был у `SubjectRequest`.
4. **Настройка, названная в правовом тексте буквально, валидируется против текста.**
   `PUT /api/admin/platform-settings` отклоняет пороги, не совпадающие с
   `TrialTermsRegistry.CurrentPromisedThresholds`: другие значения возможны только вместе с новой
   редакцией текста от `legal-counsel`.
5. **Тексты с правовой нагрузкой — отдельным файлом рядом, а не вставкой в существующий.**
   `TrialLegalNotices.cs` заведён отдельно, `LegalNotices.cs` не тронут; хеш фиксируется по
   **шаблону** текста, до подстановки значений, иначе он зависел бы от даты.
6. **Отказ не становится оракулом** (конвенция цикла 16, здесь применена дословно): текст про чужое
   прошлое не несёт ни даты, ни имени, ни намёка на существование другого аккаунта — даже когда
   сервер точно знает ответ, и даже на пути гонки.
7. **Производное от ПДн значение = ПДн:** свой ключ (не переиспользовать чужой — проверяется
   механически при старте), `KeyId` на каждой строке, значение ни в одном DTO, отдельная цель
   обработки словами юриста. Перечисление, чьё значение попало в **сырой SQL-фильтр индекса**,
   становится append-only — переставлять члены нельзя.
8. **Свой машиночитаемый контракт линтуется и сверяется с генератом с рождения:**
   `contracts/cycle18/openapi.yaml` добавлен и в `redocly lint`, и в шаг сверки
   `types:api:cycle18` в тот же цикл, а не «когда-нибудь потом».
9. 🔴 **Зелёный прогон не доказывает наличия функционала — это главный урок цикла 18 и теперь
   конвенция.** Тест **не может провалиться на коде, которого нет**: четыре несущие части цикла
   (§336, §337, §343, §333.3) отсутствовали при **770 зелёных** функциональных тестах, полностью
   зелёном юните и четырёх кругах ревью, а документы описывали их как работающие. Поэтому **до**
   объявления готовности обязательна отдельная проверка «есть ли в дереве то, что документ объявляет
   работающим»: `grep` по именам заявленных классов (`TrialLifecycleTask`, `TrialMailingWindowStarter`,
   `TrialPhoneRegistrationRule`) и `ls` по каталогу задач/правил. Этого достаточно, и это дешевле
   любого прогона.
10. **Фоновая задача и правило retention изолируют ошибки на двух уровнях — по фазе и по элементу
   пачки** (`TrialLifecycleTask`, конвенция TD-04 из `DataRetentionTask`), ходят по пачкам
   **keyset-курсором** (`Id > cursor`), а незакоммиченные правки упавшей итерации **отбрасывают**,
   чтобы они не уехали в базу с чужим `SaveChanges`. Сбой обязан быть **виден** в
   `ScheduledTaskOutcome.Error`, а не только в логах.
11. **Сигнал оператору (GlitchTip) отправляется вне открытой транзакции и вне взятой блокировки.**
   Иначе внешний вызов держит блокировку строки на время сетевого таймаута.
...
**Числа итогового прогона цикла** (снимаю с отчёта закрытия цикла, сам ничего не запускал —
запуск тестов в этой роли запрещён): функциональные **797/797** (базовая линия цикла — 735), юнит
**1522/1522**, фронтенд **618 в 87 файлах**, `tsc` чист, сборка `Release` с `-warnaserror` — 0
предупреждений / 0 ошибок, проверка непрерывности снапшотов миграций — **68** проверок, потерь
свойств нет; миграций цикл добавил **три**. ⚠️ Промежуточные числа «770/770 и 1520/1520» из прошлой
...
валидация настроек пробного периода; блоки C/D — активация, все коды отказа, гонка двух активаций,
выдача и перевыдача суперадмином, невозможность обойти отметку владельцем; блок E — витрина и
предпросмотр (`isTrial`); блоки F/G — админ-фильтр `?trial=`, карточка аккаунта, превью удаления
аккаунта. Написаны **QA по спеке**, не разработчиком.

🎯 **Второй функциональный набор — `ServiceBooking.Tests/Tests/Cycle18TrialLifecycleTests.cs`**
(1 912 строк, **26 тестов**, кейсы `CY18L-01`…`CY18L-26`, все с атрибутом `TestCase`, четыре класса
плюс общая база): хук старта окна рассылок, все четыре фазы `trial-lifecycle` (самолечение,
закрытие окна, пороги, переход во Free), фактическая доставка рассылки на триале (§333.3) и правило
уничтожения реестра номеров, включая ротацию ключа и пустое значение `Trial:PhoneKeyId`. Набор
**дописывался тремя порциями по кругам ревью**: `CY18L-01…14` (второй проход), `CY18L-15…22`
(третий), `CY18L-23…26` (четвёртый — блокеры про перезапись оплаченной строки опции, keyset-курсор и
видимость сбоя). Набор `Cycle18TrialPlanTests.cs` доращён до **35** кейсов (добавлен `CY18-C03b` —
вторая ветка гейта `PhoneVerificationUnavailable`).

**Юнит-тесты:** новые `TrialWindowTests` (11), `TrialPhoneKeyTests` (9),
`TrialActivationTermsVersionTests` (4), `SubscriptionResolverRulesTests` (25),
`OwnerSubscriptionOverLimitTests` (9); дополнены существующие `DeploymentSafetyChecksTests`
(проверки `ValidateTrialSecrets`, включая К2) и `LegalConsentFilterTests` (allow-list превью
удаления). **Фронтенд:** дополнены `api/billing.test.ts`, `api/adminBilling.test.ts`,
`api/plans.test.ts`, `pages/admin/NotificationsAdminTab.test.tsx`, новый
`utils/trialError.test.ts`.

🔴 **Ловушка, из-за которой активация триала была непокрываема тестами, и её надо помнить:** до
правки `CustomWebApplicationFactory.cs` (тестовые `Trial:PhoneKeyHmac`/`Trial:PhoneKeyId`) каждый
тестовый хост отказывал в активации fail-closed'ом по отсутствию ключа — блок активации был зелёным
и пустым. Подробнее — §6 🎯 п. 1.

🔴 **Две ловушки самого прогона, стоившие этому циклу времени — читать до первого `dotnet test`:**
1. **Два полных прогона `dotnet test` параллельно против одного docker дают ложные падения** в
   тестах, чувствительных ко времени. Признак однозначный: прогон идёт **3.5+ минуты вместо ~1.5**
   и падает **1–3 теста, не связанных с вашей правкой**. Это не регрессия — это второй прогон;
   договаривайтесь об исключительном доступе к docker (ср. `T8-3` про colima).
2. **Проверка «было ли это падение до моих правок» через `git stash` неверна, если свои правки уже
   закоммичены** — `stash` их не уберёт, и вы сравните состояние с самим собой. Нужна **чистая
   ветка** (`git worktree`/отдельный клон на базовом коммите).

⚠️ **Чего в тестах цикла нет:** юнит-тестов на `TrialActivationService` и `TrialLifecycleTask` —
оба работают напрямую с `AppDbContext`, а поднимать `DbContext` в юнит-тестах в проекте не принято
...
### 🎯 Что цикл 18 изменил в CI и в требованиях к выкату

**CI — новых шагов нет, расширены два существующих** (`.github/workflows/ci.yml`):
`redocly lint` теперь линтует **пять** спек (добавлена `contracts/cycle18/openapi.yaml` — «свой
контракт линтуется с рождения»); шаг сверки сгенерированных типов перегенерирует и
`api-cycle18.generated.ts` и падает на непустом diff (npm-скрипт `types:api:cycle18` добавлен в
`frontend/package.json`). Контракты 7/9/10/11/13/14 по-прежнему не охвачены — **C17-1**.

🔴 **Новое предусловие выката (это единственное ломающее изменение цикла).** Вне `Development`
приложение **не стартует** без пригодных `Trial:PhoneKeyHmac` (base64, ≥ 32 байт) и
`Trial:PhoneKeyId`, а также если ключ совпадает с `PhoneVerification:ExternalKeyHmac` или
`Notifications:EncryptionKey`. Переменные проведены: `TRIAL_PHONEKEY_HMAC` и `TRIAL_PHONEKEY_ID` в
`.env.production.example` (с инструкцией `openssl rand -base64 32`) и в `docker-compose.prod.yml`
(`Trial__PhoneKeyHmac`, `Trial__PhoneKeyId`, пустые дефолты, чтобы `docker-build` оставался
зелёным). 🎯 ✅ **Комментарий в `.env.production.example` приведён в соответствие** — он называет
`ValidateTrialSecrets` и падение старта на негодном/совпадающем ключе (прежний **C18-7** закрыт).
⚠️ **Ротация ключа обнуляет проверку однократности** — это по-прежнему так; но строки прежнего
`KeyId` 🎯 **теперь удаляются** правилом `TrialPhoneRegistrationRule` (К3), а не остаются в базе
...
🎯 **Цикл 18 — тоже**: его блок **C18-1…C18-13** идёт **самым первым**, перед блоком C17 (та же форма
«префикс с номером цикла»). Остальная нумерация не тронута; отметки цикла 18 на чужих пунктах стоят
по месту и помечены 🎯 — это **C5** (✅ закрыт) и **C17-1** (сузился). ⚠️ **Пункты C18-2…C18-7 закрыты
внутри самого цикла 18** (четыре круга ревью), отметки ✅ стоят по месту — не читайте их как открытый
долг; вместо них цикл оставил **C18-10…C18-13**.

---

**🎯 C18 — долг и риски, зафиксированные циклом 18 (системный тариф «Триал»)**

🔴 **Врезка, которую надо прочитать до правки перехода «триал → Free».** Правомерность схемы
«предупреждаем только плашкой в кабинете» держится на пяти условиях Т1–Т5 (§4.28), и два из них —
продуктовые, а не текстовые: **на переходе нет ни одного необратимого последствия** (превышение
лимитов = заморозка «нельзя добавлять новое», а не выключение компаний; платежей на переходе нет
вовсе) и **сообщение о переходе живёт не меньше 30 дней и не закрывается**. Как только на переходе
появится списание, удаление или реальное отключение компаний, плашки станет недостаточно и продукту
понадобится внеполосный канал связи, которого у платформы нет вовсе. Это условие, а не пожелание —
`LEGAL_REVIEW_CYCLE18.md` §5, комментарий `<!-- ЮРИСТУ -->` к п. 6.16 в `03-terms-owner.html`.

**C18-1. 🔴 Пробный тариф не сидируется — после выката функция не работает, пока суперадмин не
сделает пять действий руками.** Сидов нет ни в `Program.cs`, ни в миграциях; в БД есть только
частичный уникальный индекс по `IsSystemTrial`. Нужно: создать тариф с ценой 0, отметить его пробным
(`PUT /api/admin/plans/{id}/system-trial`), выставить лимиты (рекомендация README: 5 компаний,
25 сотрудников, 1000 МБ), расставить `PlanOptionRule` по **каждой** опции каталога и решить про
...
`TrialChannelFirstAuthorizedAtUtc`/`TrialMailingWindowEndsAtUtc`/`MailingUntilUtc`; пропущенный старт
догоняет фаза 1 задачи `trial-lifecycle`, правило резолвера перестало быть инертным. Окно после
аварийной повторной выдачи сбрасывается и не открывается задним числом. §4.28.

**C18-3. ✅ ЗАКРЫТО в цикле 18. Фоновая задача `trial-lifecycle` есть** (восьмая задача
планировщика, период 1 час, четыре фазы с изоляцией ошибок по фазе и по аккаунту и keyset-курсорами
во всех фазах). Пороговые предупреждения «за 7, 3 и 1 день» работают (`TrialBanner.tsx` достижим),
`TrialExpired`/`TrialMailingWindow` пишутся, три колонки состояния заполняются. §4.28.

**C18-4. ✅ ЗАКРЫТО в цикле 18. У реестра номеров есть правило уничтожения** —
`TrialPhoneRegistrationRule` (19-е правило retention): три года с даты предоставления (Д16,
`Retention:TrialPhoneRegistrationDays` = 1095) и удаление строк прежнего `KeyId` при ротации (К3).
Обещание Политики (п. 5.6.7) и Соглашения (п. 6.16.7) исполняется кодом. ⚠️ Пустая строка в
`Trial:PhoneKeyId` намеренно **не** считается настроенным ключом — иначе К3 вычистило бы весь реестр.

**C18-5. ✅ ЗАКРЫТО в цикле 18. Девятый код отказа `PhoneVerificationUnavailable` достижим** —
отдаётся и активацией, и чтением состояния, когда подсистема подтверждения телефона
выключена/недоступна (R7); покрыт обеими ветками (`CY18-C03`/`CY18-C03b`). Контракт и код совпадают:
девять кодов. ⚠️ Сам гейт при этом продублирован в двух местах — **C18-11**.

**C18-6. ✅ ЗАКРЫТО реализацией, а не правкой текстов.** `CHANGELOG.md`, `README.md` и
`API_DOCUMENTATION.md` описывали C18-2…C18-4 как работающие — теперь так и есть, тексты менять не
потребовалось. См. §5.4 🎯. 🔴 Но форму расхождения запомните: документы были свежие, подробные и
неправдивые, и это не ловилось прогоном (§6 🎯 п. 9).
...
чтобы не столкнуться с ними при внесении; (б) **Соглашение с владельцем в пп. 4.1 «б», 6.13.8 и
...
предыдущие черновые редакции — редакции Политики и Соглашения с владельцем подняты до
`2026-09-26-draft`. См. §8 🎯.

**C18-10. Гонка в фазе истечения СУЖЕНА, но не устранена — принятое решение с названной ценой.**
Фаза 4 `TrialLifecycleTask` перед мутацией **перечитывает тариф именно этого аккаунта**, поэтому окно
сократилось с «время обработки всей пачки» до «между этим чтением и `SaveChangesAsync` той же
итерации» — с секунд до одного обращения к базе. Полное закрытие требует **advisory-lock, взятого до
обоих действий, и перечитывания под ним**; это меняет структуру батчевой оптимизации и **сознательно
не делалось** в этом цикле. Цена уже принятого сужения: **примерно удвоение числа обращений к базе за
проход** на большом бэклоге. Последствие сохранившегося окна: тариф, назначенный админом в этот
момент, может быть переписан обратно во Free без следа в журнале. §5.1-bis п. 1.

**C18-11. Гейт «телефон подтверждён + состояние подсистемы подтверждения» продублирован в двух
местах** — `TrialActivationService` и `TrialStateReader`. Оба читают один источник, и ревью
...
поднимать `DbContext` в юнит-тестах в проекте нет: этот слой исторически проверяется функциональными
тестами (`CY18-*`, `CY18L-*`). Заводить параллельный стиль юнит-тестов в разрез с конвенцией не
стали. Риск честный: логика фаз и активации держится **только** на функциональном наборе, который
требует docker. §5.1-bis п. 3.

**C18-13. 🔴 Организационный риск, доказанный этим циклом: зелёный прогон не доказывает наличия
функционала.** Четыре несущие части цикла (§336, §337, §343, §333.3) отсутствовали в дереве при
**770 зелёных** функциональных тестах и четырёх кругах ревью — тест не может провалиться на коде,
которого нет, а §333.3 не была названа ни в архитектурных отчётах, ни в каталоге тестов. Обязательная
мера — проверка «есть ли в дереве то, что документ объявляет работающим» (`grep` по именам классов,
`ls` по каталогу задач/правил) **до** объявления готовности (§6 🎯 п. 9). Связанная ловушка:
подсистема с **fail-closed по секрету** обязана получить тестовый секрет в тестовой фабрике, иначе
её тесты зелёные и пустые (§6 🎯 п. 1). И две ловушки прогона — параллельные `dotnet test` против
одного docker и `git stash` на уже закоммиченных правках — §7 🎯.
...
✅ 🎯 **ЗАКРЫТ циклом 18 (задача D1).** Спеки циклов 13, 15 и 17 восстановлены из истории в дерево
под своими именами (`SPEC_CYCLE13_PHOTO_LOGO_MAPS.md` — `git show 4058ae9:SPEC.md`,
`SPEC_CYCLE15_BOOKING_CARD_TARIFF.md` — `f45fe56`, `SPEC_CYCLE17_C15_TECHDEBT.md` — `e9b7005`), и
**ссылки на них перенаправлены в том же коммите** (`aaf3dc9`) — в `ARCHITECTURE_CYCLE13/15/16/17.md`,
`API_CONTRACT_CYCLE15.md`, `LEGAL_REVIEW.md`, этом документе (§9 C5 и §10.5) и xml-doc пяти тестовых
файлов; по отчёту цикла — 24 ссылки. То есть требование «переименование спеки обязано включать
обновление ссылок в том же коммите» цикл 18 не только записал, но и исполнил. ⚠️ Сам корневой
`SPEC.md` цикл 18 занял собой — так что механика «занять корень и заархивировать предыдущего» не
изменилась, изменилось то, что долг больше не накапливается.
...
🎯 **Что цикл 18 изменил в §10.1/§10.2 (продуктовая и пользовательская документация).**
`CHANGELOG.md` получил **четырнадцатый** раздел «Не выпущено» — «пробный период: максимум
возможностей бесплатно, один раз, на две недели» (+272 строки, та же структура «что получает
владелец → ограничения → почему так законно → правовые документы»; счётчик разделов в шапке файла
обновлён с тринадцати на четырнадцать). `README.md` получил абзац про пробный период в разделе
возможностей и **три** абзаца в «Чего пока нет» (оповещения владельца вне кабинета; отсутствие
готовой карточки тарифа в каталоге; отсутствие абсолютной однократности), плюс обновлённые редакции
правовых документов. ⚠️ **Развёрнутая пользовательская документация (`docs/`) про пробный период не
знает вообще** — ни `docs/owner.md`, ни `docs/faq.md`, ни `docs/admin.md` цикл не трогал (в диффе
цикла файлов `docs/` нет). 🎯 ✅ **Прежнее расхождение снято:** и `CHANGELOG.md`, и `README.md`
описывали окно рассылок, пороговые предупреждения, фоновый переход и трёхлетний срок реестра как
работающие — теперь они действительно работают, тексты править не потребовалось (§5.4 🎯, **C18-6**).
Оба файла остаются честны про то, что строки тарифа в каталоге нет и что единственный канал
оповещения владельца — кабинет.
...
🎯 **Контракт цикла 18** — `API_CONTRACT_CYCLE18.md` (808 строк, **§360–§379**) и
**`contracts/cycle18/openapi.yaml`** (1428 строк) — **девятый** машиночитаемый контракт проекта
(каталогов в `contracts/` стало девять; `cycle12` по-прежнему отсутствует). Описывает **девять
маршрутов**: `GET|POST /api/billing/trial`, `POST /api/billing/trial/terms-acknowledgement`,
`PUT /api/admin/plans/{id}/system-trial`, `GET /api/admin/trial-terms/{version}`,
`POST /api/admin/billing-accounts/{accountId}/trial`, `…/trial/regrant`,
`GET /api/profile/delete-account/preview` — плюс добавочные поля у `/api/billing/subscription`,
`/api/admin/plans`, `/api/admin/platform-settings`, `/api/admin/billing-accounts`, `/api/pricing`,
`/api/admin/pricing/preview`. Контракт **линтуется `redocly` и сверяется с генератом
`api-cycle18.generated.ts` с рождения** (§8 🎯). `API_DOCUMENTATION.md` дополнен разделом «Системный
тариф „Триал“ — новое в цикле 18» (+209 строк): по эндпоинту на подраздел, таблицы кодов ответа и
**таблица девяти кодов отказа активации с фиксированным порядком проверок**. 🎯 ✅ Раздел
**соответствует коду**: девятый код (`PhoneVerificationUnavailable`) достижим (**C18-5** закрыт), окно
рассылок и фоновый переход реализованы (**C18-6** закрыт). Правка хвоста цикла в этом файле —
одна строка чек-листа приёмки (девять маршрутов, не восемь).

### 10.4 Описания тест-кейсов

**`TEST_CATALOG.md`** (🎯 ~639 КБ, 5 523 строки), Markdown, русский — человекочитаемое описание
...
🎯 **Цикл 18 добавил в каталог четыре раздела подряд — по одному на круг ревью** (+90 и затем
+356 строк):
* `## Цикл 18 — QA-написанные тест-кейсы (`CY18-`, `ServiceBooking.Tests/Tests/Cycle18TrialPlanTests.cs`)`
  — кейсы по блокам A/B/C/D/E/F/G (в файле на момент закрытия — **35** кейсов), отдельный абзац про
  **инфраструктурную правку `CustomWebApplicationFactory`**, без которой набор не мог запуститься
  (§6 🎯 п. 1), и **«Найденные баги» — семь пунктов, сдаваемых backend-developer**;
* `## Цикл 18 — QA "Вызов 2" (второй проход): три несущих части, реализованные без функциональных
  тестов (`CY18L-`, `Cycle18TrialLifecycleTests.cs`)` — `CY18L-01…14`;
* `## Цикл 18 — QA "Вызов 2" recheck (третий проход по этому же файлу)` — `CY18L-15…22`;
* `## Цикл 18 — QA, четвёртый проход (третий круг ревью, блокеры Б1/Б2/Б3 + хвосты)` —
  `CY18L-23…26`.

⚠️ **Как читать эти разделы вместе:** раздел `CY18-` снят **в середине цикла**, и его список
«найденных багов» описывает состояние на тот момент — четыре из семи багов закрыты коммитами цикла
сразу, а три (хук окна рассылок, фоновый переход, правило уничтожения реестра) были закрыты **позже**,
на втором проходе, и именно им посвящён набор `CY18L-`. Ни один из семи не остался открытым.
🔴 Четвёртая отсутствовавшая часть (§333.3, материализация строки опции) **не была названа ни в этом
каталоге, ни в отчётах** — её нашли только на четвёртом круге ревью; отсюда конвенция §6 🎯 п. 9.
Отдельного `TESTPLAN.md` или каталога `docs/testing/` с кейсами по-прежнему нет.
...
лежат в дереве под своими именами —
**`SPEC_CYCLE13_PHOTO_LOGO_MAPS.md`** (558 строк, `git show 4058ae9:SPEC.md`),
**`SPEC_CYCLE15_BOOKING_CARD_TARIFF.md`** (648 строк, `git show f45fe56:SPEC.md`) и
**`SPEC_CYCLE17_C15_TECHDEBT.md`** (405 строк, `git show e9b7005:SPEC.md`), — а ссылки «`SPEC.md` §N»
из документов этих циклов в том же коммите перенаправлены на архивные имена. ⚠️ Таблицы ниже,
начиная с «Документ цикла 17», описывают **прошлые** состояния корня: там, где в них написано
«корневой `SPEC.md`», спеку надо читать в соответствующем `SPEC_CYCLE*`-файле.
...
⚠️ **Каталога `docs/history/` по-прежнему нет**, соглашение об архиве не заведено — документы цикла
18 лежат в корне рядом с предыдущими, как и все прошлые циклы.

| Документ цикла 17 | Размер | Что это |
|---|---|---|
| 🧹 **`SPEC_CYCLE17_C15_TECHDEBT.md`** (405 строк; до архивирования циклом 18 — корневой `SPEC.md`) | — | **ЦИКЛ 17** «закрываем техдолг цикла 15», истории **US-17-01…US-17-15**. Задание цикла — блок **C15-1…C15-10** §9 этого документа. ⚠️ Раздел «Что стоит проверить с заказчиком» содержит **пять развилок**, из них первая (сработавший триггер возврата к риску Р6 по подтверждению телефона) **в объём цикла не вошла** — подсистема телефона была запрещена к правке. ⚠️ В шапке зафиксировано, что воркфлоу снова передал задание строкой `Идея продукта от пользователя: undefined` — известный дефект передачи аргументов, не пустой запрос |
...
🔁 **Состояние корня после цикла 15 — историческая справка.** Тогда корневой `SPEC.md`
(**648 строк**) был **спекой цикла 15**, а не цикла 13: цикл 15 занял корневой файл, как до него
делали циклы 11, 13 и 14 (§9 **C5**), и спека цикла 13 никуда не копировалась. ✅ **Сегодня обе
спеки лежат в дереве** — `SPEC_CYCLE15_BOOKING_CARD_TARIFF.md` и `SPEC_CYCLE13_PHOTO_LOGO_MAPS.md`
(задача D1 цикла 18), и ссылки из документов циклов 13 и 15 ведут туда, а не в корневой `SPEC.md`.
