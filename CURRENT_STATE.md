...
**Актуально по состоянию на коммит: `30d0da9` (HEAD ветки `cycle/12-legal-operator-status`),
дата: 2026-09-24.** Коммит самой этой правки ляжет поверх `30d0da9`.
...
### Миграции (🔔 62, все в `ServiceBooking.Infrastructure/Migrations/`)
...
| 📜 **`SPEC.md`** | — | **ЦИКЛ 11** «правовые тексты доводятся до состояния „публикуется одним действием“», редакция 2, ветка `cycle/11-legal-texts`. ⚠️ **Занял корневой `SPEC.md`**; спека цикла 8, которая лежала здесь раньше, архивирована в **`SPEC_CYCLE8_TEST_ISOLATION.md`** (см. §9 C5 — ровно та ловушка со ссылками, о которой предупреждал цикл 6) |
