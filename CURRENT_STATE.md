...
**Актуально по состоянию на коммит: `9864b3c` (HEAD ветки `cycle/018-trial-plan`),
дата: 2026-09-26.** Прошлая отметка была `e9b7005` (`develop`). Правка снята **на ветке цикла 18, до
мёржа в `develop`** — ветки эта правка не трогает, мёрж делает devops-engineer. **Следующий diff
отсчитывайте от той точки `develop`, куда ляжет мёрж цикла 18**, а не от `9864b3c`.

🎯 **Как читать эту редакцию (закрытие цикла 18, «системный тариф Триал»).** Это **точечное
обновление** по диапазону `e9b7005..9864b3c` — **36 коммитов, 106 файлов, +21 097 / −464 строк**;
репозиторий заново не сканировался. Метка блоков цикла — 🎯. Что искать: **§0.3** (что из §0.2
перестало быть правдой), **§3 блок 🎯** (две новые сущности, 16 новых колонок, две миграции —
файлов миграций стало **69**), **§4.28** (что цикл сделал и чего в нём нет), **§5.0-nonies**
(совместимость), **§5.1-bis** (четыре незакрытые части цикла — существенные), **§6 🎯**
(конвенции), **§7 🎯** (набор `CY18-` и ловушка тестового секрета), **§8 🎯** (CI, переменные
деплоя, предусловие О15), **§9 блок C18-1…C18-9** плюс ✅ на **C5** и уточнение **C17-1**,
§10.1/§10.3/§10.4/§10.5 (документы цикла).

🔴 **Первое, что нельзя прочитать неправильно: строки пробного тарифа в каталоге нет, и цикл её не
сидирует.** Сидов нет ни в `Program.cs`, ни в миграциях — есть только частичный уникальный индекс.
После выката пробный период **не работает вообще**, пока суперадмин руками не создаст тариф, не
отметит его пробным, не выставит лимиты и не расставит правила по **каждой** опции каталога (опция
без явного правила в триал не попадает, и добавленная в каталог позже сама туда не попадёт) —
§4.28 п. 0 и **C18-1**.

🔴 **Второе: три подсистемы из архитектуры цикла в код не вошли** — старт окна рассылок (§336.1
архитектуры), фоновая задача `trial-lifecycle` (§337) и правило уничтожения реестра номеров (§343.2).
Из-за третьей у живых персональных данных сейчас нет срока уничтожения. `CHANGELOG.md` и `README.md`
описывают все три как работающие — расхождение названо в **§5.1-bis** и **§5.4 🎯**, пункты
**C18-2…C18-4**.

🔴 **Третье, для следующего цикла, который тронет переход «триал → Free»: пять условий Т1–Т5 несут
...
§0.2 выше — снимок «до», он намеренно не переписан. По коду на `9864b3c` изменилось ровно это:
...
3. **п. 3 «срок жизни подписки — одна колонка `PaidUntil`» — по-прежнему верно, и срок триала
   ...
   **вторая дата другого смысла** — `AccountSubscriptions.MailingUntilUtc`: «рассылочные
   возможности ЭТОЙ подписки годны не дольше этой даты», правило общее для любого тарифа. Даты
...
### 🎯 Системный тариф «Триал»: две новые сущности и 16 новых колонок (цикл 18)

Всё аддитивно: ни одного `NOT NULL` на существующей таблице, ни одного переименования, ни одной
миграции данных.

**Новые колонки на существующих таблицах (16):**
...
* `AccountSubscriptions.MailingUntilUtc` — `timestamptz NULL`, «рассылочные возможности этой
  ...
  `TrialMailingUntil` намеренно: это свойство подписки, а не признак триала. Сегодня её никто не
  заполняет ненулевым значением (§5.1-bis п. 1).
* `BillingAccounts` — **14 nullable-колонок** состояния: `TrialStartedAtUtc`, `TrialEndsAtUtc`,
  ...
  `TrialMailingClosureLoggedAtUtc`. ⚠️ **Последние пять из этого списка не заполняет ни одна строка
  кода** — они под подсистемы, которые в цикл не вошли (§5.1-bis). `TrialStartedAtUtc` — отметка
  «этим аккаунтом триал использован», она не стирается ни истечением, ни сменой тарифа.
* Индекс `IX_BillingAccounts_TrialExpiry` (фильтр `TrialEndsAtUtc IS NOT NULL AND
  TrialExpiredHandledAtUtc IS NULL`) — заведён под единственную выборку фоновой задачи, которой в
  коде нет; сегодня по нему никто не ходит.
...
и по `KeyId` (под правило уничтожения — которого нет, §5.1-bis п. 3). 🔴 **Ни `FK`, ни `UserId`, ни
...
⚠️ **пишется только `TrialGranted`** — 7 и 8 не пишет никто, потому что фоновой задачи нет.

**Две миграции**, обе добавочные:
`20260925145043_AddTrialPlanFlagAndAccountTrialState` (флаг тарифа, `MailingUntilUtc`, 14 колонок,
два индекса) и `20260925145131_AddTrialGrantAndPhoneRegistry` (две таблицы с их индексами).
Снапшот (`AppDbContextModelSnapshot.cs`) закоммичен тем же изменением; файлов миграций без
`.Designer` стало **69** (было 67).
...
`TrialUniquenessCheckUnavailable`, `TrialPhoneAlreadyUsed`. ⚠️ Девятый код контракта —
`PhoneVerificationUnavailable` — **в коде не возвращается никогда** (в проекте нет сигнала «адаптер
подтверждения телефона недоступен»; названо комментарием в сервисе), см. **C18-5**. Текст отказа по
...
`AllowNotificationChannel`. ⚠️ Сегодня это правило **не срабатывает никогда**, потому что колонку
никто не заполняет (§5.1-bis п. 1).
...
— Т3), `TrialBanner.tsx` (закрываемое напоминание о пороге — ⚠️ **сегодня недостижим**, сервер
порогового предупреждения не отдаёт), `utils/trialError.ts` (14-й маппер кодов отказа).
...
**Рубильника у функции нет** — она включается наличием строки тарифа в каталоге (см. п. 0). Что из
архитектуры цикла в код **не** вошло — §5.1-bis, и это четыре существенных пункта.
...
🎯 **Дополнение цикла 18 к §5.4 — свежая документация описывает то, чего в коде нет.** Запись
цикла 18 в `CHANGELOG.md`, новые абзацы `README.md` («пробный период» и «чего пока нет») и раздел
`API_DOCUMENTATION.md` написаны по архитектуре цикла и утверждают как работающее: предупреждения за
7, 3 и 1 день; окно рассылок, которое стартует с привязкой первого номера и закрывается через 7
дней; автоматический переход «триал → Free» фоновым проходом с догоном пропущенных суток; хранение
реестра номеров три года. Ни одного из этих четырёх механизмов в коде нет (§5.1-bis, **C18-2…C18-4**,
**C18-6**). README при этом **честен** про два других факта — что строки тарифа в каталоге нет и что
единственный канал оповещения — кабинет.
...
**Одно поведенческое изменение на общем пути, сегодня нейтральное.** `SubscriptionResolver.Resolve`
теперь гасит `AllowMailing` и `AllowNotificationChannel`, если `AccountSubscriptions.MailingUntilUtc`
в прошлом. Правило общее для любого тарифа, но колонку сегодня никто не заполняет, поэтому
наблюдаемого эффекта нет — он появится ровно тогда, когда кто-нибудь начнёт её писать.
...
### 5.1-bis 🎯 Что цикл 18 задумал, но в код не внёс (дополнение к §5.1)

Четыре пункта. Все четыре подтверждены чтением кода, а не отчётами, и три из них названы самим
кодом (doc-комментарий `TrialStateReader.cs`: «neither of which this slice implements yet»).

1. **Старта окна рассылок нет.** `TrialChannelFirstAuthorizedAtUtc`, `TrialMailingWindowEndsAtUtc` и
   ненулевой `MailingUntilUtc` **не пишет ни одна строка кода**: хука на первую авторизацию канала
   (§336.1 архитектуры) в `NotificationChannelsController`/`NotificationsController` нет,
   `ChannelStateTransition` цикл не менял. Следствия: `mailingWindow` у живого триала всегда
   `NotStarted`, состояния `Running`/`EndingSoon`/`Ended` недостижимы, добавленное правило
   резолвера инертно, а рассылки внутри триала на деле живут **весь** срок триала, а не 7 дней —
   вопреки тексту условий, который владельцу показывают. **C18-2**.
2. **Фоновой задачи `trial-lifecycle` нет.** В `ServiceBooking.API/Services/Scheduling/Tasks/`
   те же **семь** файлов, что и до цикла; в `Program.cs` новая задача не регистрируется; класса с
   именем `TrialLifecycleTask` в дереве нет (единственные следы — константа актора
   `"system:trial-lifecycle"` в `TrialActors.cs` и комментарий у индекса в `AppDbContext`).
   Следствия: `SubscriptionChangeKind.TrialExpired` и `TrialMailingWindow` не пишутся никогда;
   `TrialExpiredHandledAtUtc`, `TrialWarnedAtThresholdDays`, `TrialMailingClosureLoggedAtUtc`
   остаются `null` навсегда; **пороговых предупреждений за 7, 3 и 1 день не существует** —
   `TrialStateDto.Warning` в состоянии `Active` всегда `null`, а три готовых текста юриста
   (`TrialEndingWarning7Days/3Days/1Day`) не читает никто, и `TrialBanner.tsx` недостижим. Сам
   **доступ** при этом истекает корректно (по `PaidUntil`, силами резолвера), и плашка Т3 в
   состоянии `Expired` работает — 30 дней считаются от `TrialEndsAtUtc`, потому что момента
   материализации в БД нет. **C18-3.** ⚠️ Пороговые предупреждения — не украшение: их обещает
   показанный владельцу текст условий и п. 6.16 Соглашения.
3. **Правила уничтожения реестра номеров нет.** В `Services/Retention/Rules/` — те же **18**
   файлов, `TrialPhoneRegistrationRule` отсутствует, в `Program.cs` не регистрируется, ключа
   `Retention:TrialPhoneRegistrationDays` в `appsettings.json` нет. Следствие: строки
   `TrialPhoneRegistrations` **не удаляются никогда** — ни через три года (Д16), ни при ротации
   ключа (К3), — то есть у персональных данных, которые сам цикл квалифицировал как ПДн, сейчас нет
   срока уничтожения, хотя Политика (п. 5.6.7) и Соглашение (п. 6.16.7) обещают три года.
   **C18-4** — самый неприятный из четырёх, потому что это обещание в опубликованном тексте.
4. **Код отказа `PhoneVerificationUnavailable` недостижим** — контракт и `API_DOCUMENTATION.md`
   обещают девять кодов, код возвращает восемь; текст `TrialRefusedPhoneVerificationUnavailable` не
   используется. Причина названа комментарием в сервисе: в кодовой базе нет сигнала «подсистема
   подтверждения телефона недоступна» (`Provider = "stub"` — нормальный штатный дефолт, а не
   признак аварии). **C18-5**.
...
запуск тестов в этой роли запрещён): функциональные **770/770** (было 735), юнит **1520/1520**,
фронтенд **618 в 87 файлах**, `tsc` чист, сборка `Release` с `-warnaserror` — 0 ошибок / 0
предупреждений, проверка непрерывности снапшотов миграций — 68 проверок, потерь нет.
...
тестовый хост отказывал в активации fail-closed'ом по отсутствию ключа. Подробнее — §6 🎯 п. 1.

⚠️ **Чего в тестах цикла нет — и не могло быть:** старта окна рассылок, фонового перехода «триал →
Free», пороговых предупреждений и уничтожения реестра номеров (§5.1-bis — вызывать нечего, кода
нет). Браузерного e2e в проекте по-прежнему нет вовсе: Playwright или аналога не появилось, экраны
триала покрыты только компонентно (Vitest).
...
зелёным). ⚠️ **Комментарий в `.env.production.example` устарел** — он утверждает, что проверки на
старте нет («B8 not implemented this pass»), хотя `ValidateTrialSecrets` добавлен позже в том же
цикле (**C18-7**). ⚠️ **Ротация ключа обнуляет проверку однократности** и, поскольку правила
уничтожения строк прежнего `KeyId` нет (§5.1-bis п. 3), оставляет их в базе навсегда.
...
🎯 **Цикл 18 — тоже**: его блок **C18-1…C18-9** идёт **самым первым**, перед блоком C17 (та же форма
«префикс с номером цикла»). Остальная нумерация не тронута; отметки цикла 18 на чужих пунктах стоят
по месту и помечены 🎯 — это **C5** (✅ закрыт) и **C17-1** (сузился).
...
**C18-2. Старт окна рассылок не реализован.** Хука на первую авторизацию канала нет,
`TrialChannelFirstAuthorizedAtUtc`/`TrialMailingWindowEndsAtUtc`/ненулевой `MailingUntilUtc` не
пишет никто, поэтому рассылки внутри триала фактически живут весь его срок — вопреки показанному
владельцу тексту условий. Добавленное правило `SubscriptionResolver` инертно. §5.1-bis п. 1.

**C18-3. Фоновой задачи `trial-lifecycle` нет; пороговых предупреждений «за 7, 3 и 1 день» не
существует.** Доступ истекает сам (по `PaidUntil`), но событий `TrialExpired`/`TrialMailingWindow` в
истории подписки не появляется, три колонки состояния остаются `null` навсегда, три готовых текста
юриста не читает никто, `TrialBanner.tsx` недостижим. Предупреждения обещаны текстом условий и
п. 6.16 Соглашения — то есть это долг с правовой стороной, а не только UX. §5.1-bis п. 2.

**C18-4. 🔴 У реестра номеров нет правила уничтожения.** `TrialPhoneRegistrations` не удаляется ни
через три года с даты предоставления (Д16), ни при ротации ключа (К3); правил уничтожения в проекте
по-прежнему 18, ключа `Retention:TrialPhoneRegistrationDays` в конфигурации нет. При этом Политика
(п. 5.6.7) и Соглашение (п. 6.16.7) **обещают** срок три года, а сам цикл квалифицировал эти данные
как персональные (не обезличенные). Хранение без реализованного срока — прямое расхождение с
опубликованным обещанием. §5.1-bis п. 3.

**C18-5. Контракт обещает девять кодов отказа активации, код возвращает восемь.**
`PhoneVerificationUnavailable` недостижим: в кодовой базе нет сигнала «подсистема подтверждения
телефона недоступна». Названо комментарием в `TrialActivationService`, оставлено сознательно.

**C18-6. `CHANGELOG.md`, `README.md` и `API_DOCUMENTATION.md` описывают C18-2…C18-4 как
работающие.** Это самый дорогой вид расхождения: документы свежие и подробные, поэтому доверия к
ним больше, чем к коду. См. §5.4 🎯.

**C18-7. `.env.production.example` устарел в части проверки старта** — говорит, что приложение не
падает без ключа триала («B8 not implemented this pass»), хотя `ValidateTrialSecrets` появился позже
в том же цикле и падение обеспечивает.
...
цикла файлов `docs/` нет). ⚠️ И `CHANGELOG.md`, и `README.md` описывают как работающее то, чего в
коде нет — §5.4 🎯, **C18-6**.
...
**таблица девяти кодов отказа активации с фиксированным порядком проверок**. ⚠️ Девятый код
(`PhoneVerificationUnavailable`) в коде недостижим — **C18-5**; ⚠️ и раздел описывает окно рассылок
и фоновый переход как работающие — **C18-6**.
...
🎯 **Цикл 18 добавил в каталог +90 строк** — разделом того же вида:
`## Цикл 18 — QA-написанные тест-кейсы (`CY18-`, 31 тест, `ServiceBooking.Tests/Tests/Cycle18TrialPlanTests.cs`)`
(в файле на момент закрытия цикла — 33 `[Fact]` и 1 `[Theory]`). Структура: отдельный абзац про
**инфраструктурную правку `CustomWebApplicationFactory`**, без которой набор не мог запуститься
(§6 🎯 п. 1), затем кейсы по блокам A/B/C/D/E/F/G, затем **«Найденные баги» — семь пунктов, сдаваемых
backend-developer**. ⚠️ Раздел снят **в середине цикла**: четыре из семи багов (симметрия
`system-free`, отсутствие `isTrial` в витрине, архивирование спек, внесение правовых текстов)
закрыты последующими коммитами цикла, а три (хук окна рассылок, фоновый переход, правило
уничтожения реестра) **остались открытыми** — они и стали **C18-2…C18-4**. Отдельного `TESTPLAN.md`
или каталога `docs/testing/` с кейсами по-прежнему нет.
...
| 🎯 **`ARCHITECTURE_CYCLE18.md`** | 1600 строк, **§330–§356** | Решения цикла. Точки входа: **§330** (главное в семи строках), §331 (ни одной новой зависимости), **§332 модель данных** (§332.1 флаг, §332.2 `MailingUntilUtc`, §332.3 14 колонок, §332.4 `TrialGrant` + доказательство Т1, §332.5 реестр номеров, §332.7 две миграции), **§333 почему триала нет в резолвере** (ответ на R8), §334 три замка одноразовости, §335 активация и таблица отказов, §336 окно рассылок, §337 истечение и **§337.3 инвариант Т4**, §338 тексты, §339 админка, §341 структура файлов, §342 конфигурация и секреты, §343 ПДн/ключ/ретенция (**§343.1 К1–К3**), §344 что цикл сознательно не делает, §346 тесты, **§351 инварианты для ревью**, **§352 чек-лист приёмки** (в нём девять маршрутов), §355 долг, который цикл называет и не закрывает, §356 остаточные риски заказчика. ⚠️ **§336.1, §337 и §343.2 описывают код, которого в дереве нет** — §5.1-bis |
