...
**Актуально по состоянию на коммит: рабочее дерево ветки `cycle/09-max-notifications-and-fixes` —
НЕЗАКОММИЧЕННОЕ слияние `6f39c96` (цикл 9) с `11be6bc` (`origin/develop`, мёрж цикла 11),
дата: 2026-09-23.** Когда слияние будет закоммичено, здесь должен встать его хеш — до этого момента
единственная честная точка отсчёта это пара `6f39c96` + `11be6bc`.

Документ описывает **что есть в репозитории сейчас**, без предложений по развитию.

---

🧭 **В этом документе сведены ТРИ цикла. Не приписывайте их друг другу.**
...
юридический формат и **собраны в артефакт, который читает приложение**. Осталось **пятнадцать видов
плейсхолдеров**: тринадцать заполняются из `legal.values.json` (`{{ИНН_ОПЕРАТОРА}}`,
`{{НОМЕР_УВЕДОМЛЕНИЯ_РКН}}`, `{{ПОЧТА_ДЛЯ_ОБРАЩЕНИЙ}}` и т.д. — образец в
`legal.values.example.json`), два (`{{ВЕРСИЯ_ДОКУМЕНТА}}`, `{{ДАТА_ВСТУПЛЕНИЯ_В_СИЛУ}}`) подставляет
сама команда публикации из флагов `--version`/`--effective-from`. Всё, что читает приложение,
помечено `isDraft: true`. До публикации нужны значения заказчика и вычитка практикующим юристом.
...
│                                   ДВЕНАДЦАТЬ HTML-документов юриста (01…12) + legal.json +
...
7. 📜 **Правовые документы — уже НЕ заглушки, но всё ещё черновик; черновиков одиннадцать.**
   ...
   **ожидаемое состояние**, а не недоделка. Fail-fast на `isDraft` в Production **намеренно
   ...
   Что мешает публикации: **пятнадцать видов плейсхолдеров** — тринадцать из `legal.values.json`
   (`{{НАИМЕНОВАНИЕ_ОПЕРАТОРА}}`, `{{ИНН_ОПЕРАТОРА}}`, `{{ОГРН_ОПЕРАТОРА}}`,
   `{{НОМЕР_УВЕДОМЛЕНИЯ_РКН}}`, `{{ПОЧТА_ДЛЯ_ОБРАЩЕНИЙ}}` и т.д.; четыре значения из ЕГРЮЛ, два
   появятся только после уведомления РКН, семь — решения заказчика), два
   (`{{ВЕРСИЯ_ДОКУМЕНТА}}`, `{{ДАТА_ВСТУПЛЕНИЯ_В_СИЛУ}}`) подставляет команда `publish`. Плюс
...
правил изоляции (это по отдельности сказано каждым из них ниже).
...
публикации нужны: тринадцать значений реквизитов в `legal.values.json` (четыре из ЕГРЮЛ, два — только
после уведомления РКН, семь — решения заказчика), вычитка практикующим юристом и решение по LG1.
...
| 📜 **`legal-drafts/`** | — | **ЕДИНСТВЕННЫЙ ИСТОЧНИК ПРАВДЫ по правовым текстам** (цикл 11, `ARCHITECTURE_CYCLE11.md` §102.1). Двенадцать HTML (`01-privacy-policy` … `12-guardian-confirmation`, переписаны в полный юридический формат), `legal.json` и **переписанный** `README.md`. 📜 `legal.manifest.proposed.json` **УДАЛЁН** — отвергнутое предложение цикла 5. Отличие от прошлой редакции принципиальное: `App_Data/legal/` больше **не «запечённый черновик»**, а **собираемый из этого каталога артефакт**, лежащий в git; править его руками нельзя, расхождение краснеет в CI (§8) |
