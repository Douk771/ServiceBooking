...
⚠️ **Чего в цикле нет:** записи в `API_DOCUMENTATION.md` и в `docs/**`, записи в `CHANGELOG.md`
(впервые за много циклов — см. §9 AV5), правки `README.md`, и опубликованного п. 9.8 политики
(намеренно, §9 AV1). Рубильник **ни разу не включался против живого API Яндекса** — §9 AV2.
...
**AV5. ⚠️ Цикл 13 не оставил записи ни в `CHANGELOG.md`, ни в `README.md`.** Проверено: в
`CHANGELOG.md` нет ни одного упоминания карусели, 2ГИС или верификации адреса, а `README.md` в
диапазоне `2f253ad..4058ae9` правил только цикл 14. То есть **видимые пользователю изменения шапки
карточки компании и ссылок в карты не описаны нигде, кроме документов цикла и этого файла**. Так же
не тронуты `API_DOCUMENTATION.md` (три новых эндпоинта и два новых поля `CompanyDto` там
отсутствуют) и `docs/**` (§10).
...
- ⚠️ **`CHANGELOG.md` и `README.md` цикл НЕ трогал вовсе** (§9 AV5) — для пользовательски видимых
  изменений это пробел, а не «инфраструктурный цикл без последствий».
