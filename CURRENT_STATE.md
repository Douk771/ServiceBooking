...
**C15-1. 🔴 Фронт и сборочный сканер распознают разметку подстановки ПО-РАЗНОМУ — и расходятся
...
**C15-3. 🔴 Глобальный `dotnet-ef` в окружении разработчика — 10.0.8 при проекте на EF Core 8.0.11;
...
**C15-4. ⚠️ Повреждённый снапшот у миграции `20260924074741_ResetUnverifiedPhoneNumberConfirmedMirror`.**
...
**C15-5. 🔴 Правило клиентской ОТМЕНЫ «не позже чем за 2 часа» живёт только в вёрстке.**
...
**C15-6. ⚠️ Мелочи из ревью цикла 15, оставленные не чинёными** (ни одна не блокирует функцию):
...
**C15-7. 🔴 Два продуктовых требования юриста по снятому с продажи тарифу НЕ реализованы.**
...
**C15-8. 🔴 Юридический риск, названный юристом и оставленный на решение заказчика: ручное
...
**C15-9. ⚠️ `API_DOCUMENTATION.md` и `docs/**` цикл 15 не трогал.** Справочник для внешних
...
**C15-10. ⚠️ Контракт цикла 15 есть в двух формах, но генерация типов фронта из него не заведена
вовсе.** `contracts/cycle15/openapi.yaml` (647 строк) существует и правился в цикле, однако
