...
🗺 **Как читать предыдущую редакцию (цикл 13).** Она снята **на `develop`, после мёржа цикла 13**
(`4058ae9` «Merge cycle 13 into develop — logo overlay, photo carousel, map links, address
...
**AV7. ⚠️ `npm audit --omit=dev` по-прежнему даёт 4 уязвимости, и цикл их не вносил и не устранял.**
`axios` (high), `form-data` (high), `react-router`/`react-router-dom` (moderate, open redirect ×2).
Пре-существующий долг (он же в §1 и §9 D5); цикл 13 **новых зависимостей фронтенда не добавлял**
вообще, поэтому список не изменился.
...
**V1. 🔴 ПРИНЯТЫЙ РИСК Р6: регистрация на чужой номер даёт доступ к чужим гостевым данным — включая
сведения о здоровье — и позволяет их уничтожить. Цикл закрыл только путь через СМЕНУ номера.**
...
**LG5. Требование к разработке, вытекающее из текста соглашения: при переносе компании между
...
**LG9. Уязвимости в транзитивных зависимостях тестовых проектов и фронтенда — существующий долг.**
Цикл 11 их **не вносил и не трогал файлы зависимостей** (единственная правка —
`frontend/package.json` и `ServiceBooking.UnitTests.csproj` по составу проекта, не по версиям
уязвимых пакетов). См. также D5.
...
**N9-11. Пре-существующий долг зависимостей не изменился.** `npm audit --omit=dev` даёт **четыре**
уязвимости — `axios` (high), `form-data` (high), `react-router` (moderate ×2). `package.json` в цикле
**не менялся** (кроме добавленного скрипта генерации типов), то есть цикл их не трогал и не усугубил.
Тот же пункт, что §9 D5.
