...
**V4. Выпущено ВЫКЛЮЧЕННЫМ — рубильник не переведён, и наружу подсистема ни разу не работала.**
`PhoneVerification:Provider = "stub"` во всех закоммиченных конфигурациях. Следствия, которые надо
держать в голове: **ни один вызов к `platform-api2.max.ru` не делался ни разу** — формат подписи
`vcf_info` (hex или base64), точная форма апдейтов `bot_started`/`message_created` и поведение
`subscribe` проверены только по разведданным и приняты «оба варианта» там, где было неясно
(`MaxContactSignature` принимает и hex, и base64). Первое включение — это первая встреча кода
с настоящей платформой, а не «проверка, что всё работает». Отдельно: гейт `change-phone` при
выключенной подсистеме отвечает 409 **всегда**, то есть смена номера на номер с гостевыми записями
сегодня невозможна ни для кого.
