...
**Актуально по состоянию на коммит: `9729beb`, дата: 2026-09-25.**
Прошлая отметка была `4058ae9`. Снято на ветке `cycle/016-tech-debt-cleanup`, чей HEAD **побитово
равен** `9729beb` (`develop`): кода цикла 16 ещё нет, диапазон `9729beb..HEAD` пуст, поэтому
пересканировать нечего — правка этой редакции не про новый код.
...
- **§9.20 «логика прав размазана по приватным копиям `CanManageCompany`» — ЧАСТИЧНО.** Появился
  `ServiceBooking.API/Services/CompanyMembership.cs`, но `CanManageCompany` по-прежнему упоминается
  в шести контроллерах (`AdminController`, `CompaniesController`, `CompanyNotificationsController`,
  `ServicesController`, `CompanyAddressController`, `MailingController`).
- **§9.17 «пагинация `GET /api/masters/clients` работает в памяти» — похоже, ЗАКРЫТ (US-49).**
  В `MastersController.cs:141-144` стоит комментарий, что пагинация выполняется `ROW_NUMBER()`-запросом
  на стороне БД. Пункт в старом списке отметки о закрытии не имеет — это расхождение самого документа.
- **§9.16 «`Booking.ClientDeleted` фактически мёртв» — устарел наполовину.** Флаг теперь не только
  проставляется в `ProfileController.DeleteAccount` (`:461`), но и является рабочим признаком для
  `Services/Retention/Rules/BookingPersonalizationRule.cs`. В UI он по-прежнему не отображается.
...
- **§9.29 «валидация DTO неполна» — ОТКРЫТ как минимум в названном месте.**
  `MailingController.cs:73` — `public record SendMailDto(string Subject, string Message);`, без
  единого атрибута валидации.
...
**C15-2. 🔴 `npm audit`: 13 уязвимостей, 6 из них high — существующий долг, не регресс цикла.**
