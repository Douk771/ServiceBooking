...
**Актуально по состоянию на коммит: `2f253ad` (HEAD ветки `cycle/14-phone-verification-max`),
дата: 2026-09-24.** Коммит самой этой правки ляжет поверх `2f253ad`.
Прошлая редакция была снята на `30d0da9` (ветка цикла 12) — она влита в `develop` (`89ab74e`), и
ветка цикла 14 содержит `develop` целиком (`git merge-base` = `89ab74e`), поэтому ни один блок
циклов 9–12 переписывать не пришлось.

📱 **Как читать эту редакцию.** Она снята на **ветке цикла 14** (подтверждение телефона через бота
...
### Миграции (📱 62 на этой ветке, все в `ServiceBooking.Infrastructure/Migrations/`)
...
- 🔬 **`SPEC.md` в корне — это теперь спека ЦИКЛА 14** (подтверждение телефона через бота MAX).
  ...
  из документов циклов 8 и 11 без правки ведут в чужую спеку — ровно ловушка §9 C5, теперь уже
  в третий раз. **Цикл 14 дополнительно кладёт копию своей спеки в `SPEC_CYCLE14_PHONE_VERIFICATION.md`
  в конце цикла**, не дожидаясь, пока её заархивирует следующий цикл: корневой `SPEC.md` — общий
  ресурс нескольких параллельных веток, см. следующий пункт.
...
|---|---|---|---|---|
| Юнит-тесты бэкенда | `ServiceBooking.UnitTests` | ничего | `dotnet test ServiceBooking.UnitTests` | 🔔 **1 077** по статическому счёту на `6f39c96` (было 942) · 📸 **942** (цикл 10) · 📜 **1004** на ветке цикла 11 |
| **Функциональные (API) тесты** | `ServiceBooking.Tests` | 🔬 **запущенный Docker** (не PostgreSQL!) | `dotnet test ServiceBooking.Tests` | 🔔 **620** по статическому счёту на `6f39c96`, 619 реальных (было 605) · 📸 **603** (цикл 10) · 📜 **554** на ветке цикла 11 |
| Тесты фронтенда | `frontend/src/**/*.test.ts(x)` | Node 20 | `npm run test:run` (в `frontend/`) | 🔔 **419** тестов, 67 файлов на `6f39c96` (было 369 / 57) · 📸 **369**, 57 файлов (цикл 10) · 📜 **359** на ветке цикла 11 |
...
📱 **Цикл 14 — тоже**: его блок идёт **самым первым**, с номерами **V1…V6** (`V` — verification).
...
Полный разбор — `SPEC.md` §6.4 и его копия `SPEC_CYCLE14_PHONE_VERIFICATION.md` §6.4; здесь —
...
- ⚠️ **Адресация намеренно по именам, а не по номерам строк** (`SPEC.md` §6.4 ссылается на строки
...
**V6. Мёртвый сгенерированный файл типов с именем от прежнего номера цикла.**
`frontend/src/types/api-cycle12.generated.ts` (998 строк) лежит в репозитории, но **ни одним модулем
не импортируется**, а скрипт `npm run types:api:cycle14` пишет в `api-cycle14.generated.ts`, которого
в дереве нет. Типы подсистемы во фронтенде объявлены руками в `frontend/src/types/index.ts`. То есть
расхождение `contracts/cycle14/openapi.yaml` с фронтендом не проверяется ничем — та же болезнь, что
у цикла 7 (§1), плюс осиротевший файл со старым номером. Тот же остаток прежней нумерации —
...
- **Свои документы цикла есть, и их четыре:** корневой **`SPEC.md`** (теперь это спека цикла 14),
...
|---|---|---|
| 📱 **`SPEC.md`** | 1 205 строк | **ЦИКЛ 14** «подтверждение владения номером телефона через бота MAX», истории **US-14-01…US-14-17**, решения заказчика **Р1…Р6**. ⚠️ **Занял корневой `SPEC.md`** (третий цикл подряд, §9 C5); спека цикла 11, лежавшая здесь раньше, заархивирована в **`SPEC_CYCLE11_LEGAL_TEXTS.md`**. **§6.4 — принятый риск Р6**, из которого вырос §9 **V1**: читать до любой правки выгрузки, удаления аккаунта и регистрации. §0.5 — почему подсистема выпускается выключенной |
| 📱 **`SPEC_CYCLE14_PHONE_VERIFICATION.md`** | 1 205 строк | **Копия спеки цикла 14, положенная самим циклом**, чтобы корневой `SPEC.md` можно было занять следующему циклу, не теряя ссылок. Содержимое совпадает с `SPEC.md` на момент `2f253ad`; ссылки вида «SPEC.md §6.4» верны для обоих файлов |
