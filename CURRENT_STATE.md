...
юридический формат и **собраны в артефакт, который читает приложение**. Осталось **четырнадцать видов
плейсхолдеров**: двенадцать заполняются из `legal.values.json` (`{{ИНН_ОПЕРАТОРА}}`,
...
7. 📜 **Правовые документы — уже НЕ заглушки, но всё ещё черновик; черновиков одиннадцать.**
   ...
   **ожидаемое состояние**, а не недоделка. Fail-fast на `isDraft` в Production **намеренно
   ...
   Что мешает публикации: **четырнадцать видов плейсхолдеров** — двенадцать из `legal.values.json`
   (`{{НАИМЕНОВАНИЕ_ОПЕРАТОРА}}`, `{{ИНН_ОПЕРАТОРА}}`,
   `{{НОМЕР_УВЕДОМЛЕНИЯ_РКН}}`, `{{ПОЧТА_ДЛЯ_ОБРАЩЕНИЙ}}` и т.д.; три значения из ЕГРЮЛ, два
...
|---|---|---|---|---|
| Юнит-тесты бэкенда | `ServiceBooking.UnitTests` | ничего | `dotnet test ServiceBooking.UnitTests` | 📸 **942** (цикл 10) / 📜 **1004** на ветке цикла 11 |
| **Функциональные (API) тесты** | `ServiceBooking.Tests` | 🔬 **запущенный Docker** (не PostgreSQL!) | `dotnet test ServiceBooking.Tests` | 📸 **603** (цикл 10) / 📜 **554** на ветке цикла 11 |
| Тесты фронтенда | `frontend/src/**/*.test.ts(x)` | Node 20 | `npm run test:run` (в `frontend/`) | 📸 **369**, 57 файлов (цикл 10) / 📜 **359** на ветке цикла 11 |
...
публикации нужны: двенадцать значений реквизитов в `legal.values.json` (три из ЕГРЮЛ, два — только
...
из которых не решается кодом:
  ...
  5. **четырнадцать видов плейсхолдеров** в двенадцати документах юриста не заполнены (§5.1 п. 7):
