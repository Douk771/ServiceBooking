...
/// <summary>
/// Reads and validates <c>legal.values.json</c> against the 12 required keys of
...
{
    /// <summary>The 12 required keys, in the exact order legal-values.schema.json lists them —
    ...
    public static PlaceholderValues Load(string path)
    {
        ...
        using (doc)
        {
            ...
            foreach (var property in valuesElement.EnumerateObject())
            {
                ...
                {
                    problems.Add($"{property.Name}: неизвестный ключ, не входит в 12 обязательных реквизитов.");
                    continue;
                }
