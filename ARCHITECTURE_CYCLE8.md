...
|---|---|---|
| `dotnet test ServiceBooking.Tests`, локально (медиана 3) | _заполнить_ | +30 с абсолютных |
| `dotnet test ServiceBooking.UnitTests`, локально | _заполнить_ | **0** (набор не должен получить БД вообще) |
| Джоб `backend` в CI, шаг «Functional tests» | _заполнить_ (из логов последнего зелёного прогона на `7b382d8`) | +20 % |
| Джоб `backend` целиком | _заполнить_ | +20 % |
