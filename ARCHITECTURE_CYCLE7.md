# ARCHITECTURE — цикл 7 ServiceBooking: биллинг-аккаунт, каталог опций, публичный прайс

> **Переименование при мердже в `develop` (2026-09-23).** Архитектор изначально назвал этот файл
> `ARCHITECTURE_CYCLE5.md`, сославшись на «продуктовую нумерацию» — это оказалось ошибкой: в
> `develop` номер 5 уже занят циклом `cycle/05-legal-compliance` (правовые основания продукта).
> Файл переименован в `ARCHITECTURE_CYCLE7.md` вслед за именем ветки `cycle/07-pricing-model-rework`;
> разделы (§41 и далее) не перенумеровывались, только имя файла.
>
...
> тоже цикл 7 — см. переименование выше). Отправная точка — `develop` @ `7ab28b2`.
>
> **Вход:** `SPEC.md` ред. 2 и `CURRENT_STATE.md` @ `7a36543`.
> **Выход для команды:** этот файл + `API_CONTRACT_CYCLE7.md` + `contracts/cycle7/openapi.yaml`.
...
| Машиночитаемый контракт | **OpenAPI 3.0.3 файлом в репозитории** (`contracts/cycle7/openapi.yaml`), Swashbuckle остаётся только для Dev-Swagger | Схема нужна **до** кода, чтобы FE и QA работали параллельно (§57) |
...
который был бы красным до цикла 7**: после смены ответственного назначение на номер существует, а
...
contracts/cycle7/openapi.yaml             🆕 машиночитаемый контракт
```
...
├── types/pricing-api.d.ts 🆕 сгенерирован из contracts/cycle7/openapi.yaml
...
`npm run types:api` → `openapi-typescript contracts/cycle7/openapi.yaml -o src/types/pricing-api.d.ts`.
Файл **коммитится**, скрипт запускается вручную при изменении контракта. Для DTO цикла 7 ручные
...
`contracts/cycle7/openapi.yaml` — **OpenAPI 3.0.3, источник истины по форме интерфейса.**
`API_CONTRACT_CYCLE7.md` объясняет смысл и правила; YAML фиксирует форму. Расхождение между ними —
дефект документа, и правится YAML-первым.

| Роль | Как пользуется |
|---|---|
| frontend | `npx @stoplight/prism mock contracts/cycle7/openapi.yaml --port 4010` → работающий мок **в первый же день**, `VITE_API_TARGET=http://localhost:4010 npm run dev`. Плюс генерация типов (§56.3) |
| backend | пишет контроллеры под схемы; имена полей и коды берёт оттуда, а не из головы |
| QA | контрактный прогон **готовым инструментом, без ручных тестов**: `schemathesis run contracts/cycle7/openapi.yaml --base-url http://localhost:5000 --checks all`. Проверяется, что фактические ответы соответствуют схеме — то есть что BE и FE реально сошлись по форме, а не на словах в отчётах |
