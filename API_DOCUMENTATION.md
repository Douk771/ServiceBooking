...
### Системный тариф «Триал» — **новое в цикле 18**

Источник — `API_CONTRACT_CYCLE18.md` §360–§379 и `contracts/cycle18/openapi.yaml` (версия 13.1.0).
Полная схема ответов — в openapi-файле; ниже — смысл и правила, которые формой не выражаются.

🔴 **Правовая нагрузка текстов.** Все перечисленные ниже поля (`message`, `activationTerms.text`,
`warning.text`, `mailingWindow.text`, `planChangeNotice`, `usage.overLimitText`,
`trialRegistryNotice`, а также `message` в `TrialRefusalDto`) сервер отдаёт как **готовый русский
текст целиком**. Клиент обязан печатать его **дословно** — не сокращать, не склеивать из кусочков и
не подставлять собственную формулировку вместо ошибки сети/парсинга. Эти тексты — согласованные
юристом формулировки, исполняющие конкретные пункты пользовательского соглашения; своя редакция того
же смысла юридически не эквивалентна серверной.

#### `GET /api/billing/trial`

`[Authorize]`. Отвечает **200** `TrialStateDto` (доступен ли триал, а если нет — почему; состояние
уже идущего или уже использованного триала; текст условий активации с версией и хешем);
**404** (пустое тело) — у пользователя нет биллинг-аккаунта и он не владелец; **401** — нет токена.

`state` — одно из `Available | Unavailable | Active | Expired`. Фронт не вычисляет доступность,
остаток дней или состояние окна рассылок сам — это всегда поля ответа (`refusalCode`, `daysLeft`,
`mailingWindow.state` и т.д.), а не производные от других полей на клиенте. Объект `trial` не
становится `null`, если триал у аккаунта когда-либо был: при `state == "Expired"` в ответе целиком
присутствуют `endsAt` и `warning` — иначе сообщение о переходе на бесплатный тариф пропадёт с экрана
раньше срока, гарантированного клиенту (см. `warning` ниже).

`activationTerms` фиксирует показ условий активации (версия, sha256 точных байтов текста, сам текст,
`acknowledgementRequired`, `shownAt`/`acknowledgedAt`). `acknowledgementRequired == true` только когда
триал выдал суперадмин и владелец условия ещё не подтвердил — до подтверждения кабинет обязан
показывать текст блоком, который нельзя закрыть.

#### `POST /api/billing/trial` — активация владельцем

`[Authorize]`, лимит `trial-activate` (5 попыток в сутки). Тело:

```json
{ "termsVersion": "2026-09-26" }
```

Единственное поле в теле — оно не запрашивает другой срок или состав триала (это решает платформа), а
**подтверждает**, какую редакцию условий клиент только что показал человеку. Единственное допустимое
значение — текущая версия с сервера (та же, что в `activationTerms.version`); любое другое (устаревшая
редакция, неизвестная строка, пустая строка, отсутствующее тело) — **409 `TrialTermsVersionMismatch`**.

| Код | Тело | Когда |
|---|---|---|
| **200** | `OwnerSubscriptionDto` (весь объект, `trial` уже заполнен) | триал активирован |
| **400** | строка | тело отсутствует или `termsVersion` пустой |
| **404** | пустое | у пользователя нет биллинг-аккаунта (не владелец); эндпоинт его не создаёт |
| **409** | `TrialRefusalDto` | одна из девяти причин отказа — см. ниже |
| **429** | строка | больше 5 попыток в сутки |
| **401 / 451** | пустое | нет токена / нужно принять новую редакцию документов |

Повторный вызов после успешной активации — не «молча ок», а **409** (`TrialAlreadyActive` или
`TrialAlreadyUsed`): двойной клик гарантированно не выдаёт второй триал.

#### `POST /api/billing/trial/terms-acknowledgement`

`[Authorize]`. Тело — `{ "termsVersion": "2026-09-26" }`. Нужен только для одного случая: триал выдал
**суперадмин**, и владелец текста условий ещё не видел. Пока подтверждения нет,
`activationTerms.acknowledgementRequired == true`.

| Код | Тело | Когда |
|---|---|---|
| **200** | `TrialStateDto` | подтверждение записано (или уже было записано раньше — идемпотентно) |
| **400** | строка | `termsVersion` пустой |
| **404** | пустое | у аккаунта не было ни одной выдачи триала — подтверждать нечего |
| **409** | `TrialRefusalDto` (`TrialTermsVersionMismatch`) | присланная версия не текущая |
| **401 / 451** | пустое | нет токена / нужно принять новую редакцию документов |

Отметка подтверждения ставится один раз и не перезаписывается: повторный вызов отвечает 200, но не
двигает `acknowledgedAt`.

#### Девять кодов отказа активации

Возвращаются в `TrialRefusalDto` (`POST /api/billing/trial`, `POST /api/admin/billing-accounts/{id}/trial`,
`POST /api/admin/billing-accounts/{id}/trial/regrant`) как `{ "code": "...", "message": "..." }`.
**Порядок проверок фиксирован и является частью контракта:** сначала форма запроса, потом состояние
платформы, потом собственное прошлое аккаунта, потом чужое. Клиент не имеет права переупорядочивать
ветвление по подстроке текста — только по `code`.

| № | `code` | Смысл |
|---|---|---|
| 0 | `TrialTermsVersionMismatch` | клиент показал не текущую редакцию условий активации — фиксацию показа выполнить невозможно, триал не выдаётся |
| 1 | `TrialNotOffered` | системного тарифа-триала нет, он неактивен, снят с витрины или его настройки повреждены |
| 2 | `TrialAlreadyActive` | у аккаунта уже идёт активный триал |
| 3 | `AlreadyOnPaidPlan` | у аккаунта есть действующая платная подписка |
| 4 | `TrialAlreadyUsed` | триал уже был использован этим аккаунтом (текст называет дату — это собственное прошлое аккаунта) |
| 5 | `PhoneVerificationUnavailable` | номер телефона владельца не подтверждён, и при этом подсистема подтверждения номера выключена (если номер уже подтверждён, состояние подсистемы значения не имеет) |
| 6 | `PhoneNotVerified` | номер телефона владельца не подтверждён |
| 7 | `TrialUniquenessCheckUnavailable` | проверка однократности триала недоступна (нет ключа/сервис не отвечает) — отказ fail-closed, а не пропуск проверки |
| 8 | `TrialPhoneAlreadyUsed` | этот номер телефона уже получал триал на другом аккаунте (текст **не** называет дату, имя или сам факт существования другого аккаунта — ответ не должен становиться оракулом) |

🔴 **У каждого кода сервер отдаёт готовый русский текст в `message`, и клиент обязан печатать его
дословно, а не сочинять свою формулировку** — это требование юриста (`Services/Billing/TrialLegalNotices.cs`):
тексты несут правовую нагрузку и являются частью исполнения условий пользовательского соглашения.
Ветвиться в интерфейсе следует по `code`. Коды 4–8 «обходятся» аварийной повторной выдачей
(`POST .../trial/regrant`), коды 0–3 и 7 — нет ни при каких обстоятельствах, включая эту повторную выдачу.

Название кода 7 — `TrialUniquenessCheckUnavailable`, а не `TrialAntifraudUnavailable`: слова
«антифрод», «акция» и «злоупотребление» в кодах, текстах и журналах намеренно не используются
(решение Д17 цикла 18).

#### `GET /api/billing/subscription`, `POST /api/billing/subscription/request` — добавочные поля

Форма `OwnerSubscriptionDto` прежняя, добавлено поле `trial` (`TrialStateDto` или `null` — только
когда триала у аккаунта никогда не было и он недоступен) и три поля в `usage`:
...
```jsonc
{
  ...
  "usage": {
    ...
    "overLimitCompanies": 4,
    "overLimitEmployees": 24,
    "overLimitText": "Бесплатный тариф допускает компаний: 1, сотрудников: 1. Сейчас у вас компаний: 5, сотрудников: 25 — это больше лимита. Ничего из созданного не удалено и не отключено: …"
  }
}
```

🔴 **`overLimitCompanies` / `overLimitEmployees` / `overLimitText` заполняются только когда
эффективный тариф аккаунта прямо сейчас — базовый системный бесплатный тариф** (триал закончился без
перехода на платный, подписка истекла, либо аккаунт подписан на сам системный Free). Во всех
остальных случаях (в том числе когда фактическое использование превышает лимиты какого-то *другого*,
например урезанного администратором платного тарифа) — `0 / 0 / null`. Смешивать эти два случая
нельзя: текст `overLimitText` жёстко сформулирован как «Бесплатный тариф допускает…» и был бы неверен
для не-Free тарифа. (Это уточнение внесено последней правкой цикла 18 — до неё поле могло указывать
числа превышения по одному тарифу, а текст цитировать лимиты другого.)

Аккаунт, который не на триале и не превысил лимиты бесплатного тарифа, получает ответ, побайтово
совпадающий с тем, что отдавался до цикла 18, кроме `trial: null` и трёх полей со значениями
`0/0/null`.

Экран заявки на платный тариф (`POST /api/billing/subscription/request`) обязан до подтверждения
показать `trial.planChangeNotice`, если `trial.state == "Active"`: остаток дней активного триала
сгорает при смене тарифа, и текст об этом предупреждает заранее. Отдельного маршрута под этот текст
нет — он приходит тем же ответом `GET /api/billing/subscription`, который уже рисует экран.

#### `GET /api/profile/delete-account/preview` — **новое в цикле 18**

`[Authorize]`. Отвечает **200** `AccountDeletionPreviewDto`:
...
**401** — нет токена. `trialRegistryNotice` непустой **только** у владельца, которому пробный период
когда-либо предоставлялся; иначе `null`. Показывается перед подтверждением удаления аккаунта — рядом
с уже существующим предупреждением об удалении гостевых данных, а не вместо него.

#### Админка: выдача и повторная выдача триала (`SuperAdmin`)

* `POST /api/admin/billing-accounts/{accountId}/trial` — суперадмин выдаёт триал аккаунту напрямую;
  тело отсутствует; **200** — карточка биллинг-аккаунта с заполненным `trial`; **409** —
  `TrialRefusalDto` (те же девять кодов, кроме `TrialTermsVersionMismatch`, которого на этом пути быть
  не может); **404** — аккаунта нет.
* `POST /api/admin/billing-accounts/{accountId}/trial/regrant` — аварийная повторная выдача, отдельный
  маршрут (не флаг у предыдущего запроса), обязательная причина в теле:
  ...
  | Код | Тело | Когда |
  |---|---|---|
  | **200** | карточка биллинг-аккаунта | выдано повторно, отметка одноразовости обойдена явно |
  | **400** | строка | `reason` пустой, только пробелы или длиннее 500 символов |
  | **409** | `TrialRefusalDto` | только коды `TrialNotOffered`, `TrialAlreadyActive`, `AlreadyOnPaidPlan`, `TrialUniquenessCheckUnavailable` — эти четыре не обходятся никогда |
  | **404** | пустое | аккаунта нет |

  Причина попадает в журнал выдач вместе с автором, адресатом, временем и версией текста условий.

Выдача суперадмином (в отличие от самостоятельной активации) не показывает владельцу текст условий
до выдачи: `termsShownAt` в журнале остаётся `null`, пока владелец не вызовет
`POST /api/billing/trial/terms-acknowledgement`.

#### `PUT /api/admin/plans/{id}/system-trial` — **новое в цикле 18**

`[Authorize(Roles = "SuperAdmin")]`. Тело `{ "isSystemTrial": true }`. Ответ **200** — `AdminPlanDto`
(с новыми полями `isSystemTrial` и `optionCoverage`).

| Код | Тело | Когда |
|---|---|---|
| **400** | строка | у тарифа `pricePerMonth != 0` — триал бесплатен по определению |
| **409** | строка | триалом уже помечен другой тариф, либо этот тариф уже помечен как системный бесплатный, либо гонка при сохранении |
| **404** | пустое | тарифа нет |

Повторный вызов с тем же значением — **200** без изменений (та же идемпотентность, что у флага
`system-free`). В теле обычной правки тарифа (`AdminPlanInput`) поля `isSystemTrial` нет — флаг
меняется только этим отдельным маршрутом.

#### Прочие добавления цикла 18 (форма — см. `contracts/cycle18/openapi.yaml`)

* `GET /api/admin/trial-terms/{version}` — архивная редакция текста активации триала по версии
  (`SuperAdmin`); нужна, чтобы доказать постфактум, какой именно текст был показан владельцу в момент
  выдачи. **404**, если такой версии нет в реестре.
* `GET /api/pricing`, `GET /api/admin/pricing/preview` — карточка тарифа получила `isTrial: boolean`
  рядом с существующим `isFree`.
* `GET /api/admin/billing-accounts` — новый необязательный фильтр `?trial=never|active|used`; элементы
  списка получили `trialState` и `trialEndsAt`.
* `GET /api/admin/billing-accounts/{accountId}` — объект `trial` (состояние, снапшот длительности и
  порогов предупреждений на момент выдачи, история всех выдач `grants` с версией и хешем показанного
  текста); номера телефона и его хеша в ответе нет ни в одном виде.
* `GET /api/admin/platform-settings`, `PUT /api/admin/platform-settings` — три новых поля
  (`trialDurationDays`, `trialMailingWindowDays`, `trialWarningThresholdsDays`); правка действует
