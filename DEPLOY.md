...
### Заголовки против встраивания в чужой iframe

`deploy/nginx/ezbook.conf` явно запрещает встраивание всего приложения в `<iframe>` на чужих
сайтах (`X-Frame-Options: DENY` + `Content-Security-Policy: frame-ancestors 'none'`) — кроме
одного маршрута, `/embed/*` (виджет записи, US-03), для которого встраивание — единственный
смысл существования страницы, и заголовки туда намеренно не добавляются. Если добавляете новый
`location` в конфиг — не забудьте, что он унаследует поведение того блока, в который попадёт
(`/embed/` без ограничения, `/` — с ним).
