...
| `JWT_KEY`, `POSTGRES_PASSWORD`, `SUPERADMIN_PASSWORD`, `GLITCHTIP_SECRET_KEY`, `GLITCHTIP_POSTGRES_PASSWORD` | `.env` на машине | генерируются заново (`openssl rand`), но смена `JWT_KEY` разлогинит всех, а смена пароля Postgres требует правки и в базе |
