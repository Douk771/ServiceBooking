...
**Шаг 10.2a. Проверки перед выкатом миграции биллинг-аккаунтов (цикл 5, ARCHITECTURE_CYCLE5.md
§54.5).** Обязательный пункт чек-листа для релиза, несущего `AddCoTenancyConstraints` и соседние
миграции (`BackfillBillingAccounts`, `BackfillChannelBillingAccounts`) — единственный шаг цикла 5,
...
#    без единой колонки цикла 5 (сам это определяет и печатает SKIPPED там, где рано):
...
проверка ещё не применима — колонка цикла 5 появится только после `AddBillingAccounts`») — можно
