...
| `WEBPUSH_VAPID_PUBLIC_KEY`/`WEBPUSH_VAPID_PRIVATE_KEY` (VAPID-пара, cycle 9) | `.env` на машине; канал выпущен **выключенным** (`Provider=logging`) — на данный момент значений нет и генерировать их не требуется до включения канала | генерируется заново `npx web-push generate-vapid-keys` в любой момент — в отличие от `NOTIFICATIONS_ENCRYPTION_KEY` НЕ шифрует данные на машине, но обнуляет все существующие браузерные подписки мастеров разом; план смены — §17.2 |
...
19. §17 — Web Push мастеру: достижимость push-сервисов, VAPID-ключи
...
```bash
cd /opt/ezbook/app
cp legal.values.example.json legal.values.json   # заполните 10 обязательных значений перед публикацией
...
Из 12 допустимых ключей `legal.values.json` обязательны 10; `НОМЕР_УВЕДОМЛЕНИЯ_РКН` и
`ДАТА_УВЕДОМЛЕНИЯ_РКН` — необязательные (ARCHITECTURE_CYCLE11.md §103.3). Уведомление в Роскомнадзор
подаётся сразу, а само внесение в реестр занимает до 30 дней, и публикация регистрационного номера
не является требованием закона — поэтому ждать месяц ради этой строки не нужно: публикуйте с
остальными 10 значениями, оставив эти два ключа в файле отсутствующими (не пустой строкой — пустая
строка у присутствующего ключа по-прежнему ошибка формата). `publish` при этом не подставляет пустое
значение вместо номера/даты, а убирает из текста весь пункт, где они упоминаются, целиком — когда
номер появится в реестре, опубликуйте новую версию документа с уже заполненными обоими значениями,
и пункт вернётся. `legal status`/`GET /api/admin/legal/readiness` показывают такие отсутствующие
значения отдельной строкой (`placeholders[].optional: true, valuePresent: false`) — это не блокер, а
предупреждение о том, что пункт про реестр не попадёт в публикуемый текст.
...
## 17. Web Push мастеру (cycle 9, проход C — ARCHITECTURE_CYCLE9.md §105)

### 17.1. C0 — достижимость push-сервисов браузеров (§105.1, риск R10)

Web Push не доставляется нами напрямую — каждое сообщение уходит через push-сервис самого
браузера (FCM для Chrome/Edge/Firefox, Apple Push для Safari). Если с боевой машины исходящие до
них не проходят, весь блок Web Push бессмысленен в текущем виде вне зависимости от того, что код
на бэкенде уже написан и оттестирован.

**Проверено 2026-09-23**, по SSH, прямо с целевой машины (`ezbook.ru` / `192.168.0.93` — это же
и стенд `develop`, и будущий прод: см. «Единая целевая машина» в §9.3, второй машины пока нет,
поэтому отдельной проверки «со стенда» не требуется — она была бы проверкой того же самого хоста):

```bash
ssh ezbook '
for url in https://fcm.googleapis.com https://android.googleapis.com \
           https://web.push.apple.com https://api.push.apple.com; do
  echo "$url -> $(curl -s -o /dev/null -w "%{http_code}" --max-time 8 "$url")"
done
getent hosts fcm.googleapis.com api.push.apple.com
'
```
...
```
https://fcm.googleapis.com -> 404
https://android.googleapis.com -> 404
https://web.push.apple.com -> 405
https://api.push.apple.com -> 405
```

`404`/`405` здесь — ожидаемый и достаточный результат: это не эндпоинты для ручных GET-запросов
(реальная доставка идёт на URL конкретной подписки, полученный при подписке браузера, а не на эти
базовые адреса), проверка смотрит не на код ответа, а на то, что TCP/TLS до хоста вообще
устанавливается и DNS резолвит его через публичные адреса (`getent hosts` выше подтвердил резолв
обоих через настоящие IP Google/Apple, не через какой-то перехватывающий прокси). Ни таймаутов, ни
`Connection refused`, ни редиректов на страницу блокировки провайдера не было ни на одном из
четырёх адресов — фаервол/сетевая политика машины исходящий HTTPS до push-инфраструктуры Google и
Apple не режет.

**Вывод: с этой машины FCM и Apple Push достижимы, блок Web Push технически может доставлять
уведомления.** Если при перепроверке (после смены оператора/провайдера/сетевой политики) хотя бы
один из четырёх адресов перестанет отвечать HTTPS-рукопожатием — это блокер для канала, а не повод
городить обход: Provider остаётся `logging` до починки сети, продукт без push полностью
работоспособен и без него (см. §17.2 про то, что канал и так выпускается выключенным).

Перепроверяйте эту достижимость тем же способом при следующей ревизии сети машины (смена
провайдера, включение/правка фаервола, миграция на отдельную прод-машину из §9.3) — не полагайтесь
на результат 2026-09-23 бессрочно.

### 17.2. C1 — VAPID-ключи (§105.2, риск R12)

Web Push выпускается в этом цикле **выключенным**: `Notifications:StaffPush:Provider` в
`appsettings.json` остаётся `logging`, а `VapidPrivateKey`/`VapidPublicKey`/`VapidSubject` —
пустыми (тот же приём, что и с `NOTIFICATIONS_PROVIDER=logging` для WhatsApp-каналов — см. §3).
Переменные окружения `WEBPUSH_*`, которыми эти три поля реально подставляются в Production, уже
описаны в `.env.production.example` рядом с остальными секретами Notifications, тем же приёмом,
что и `NOTIFICATIONS_ENCRYPTION_KEY`: сгенерировать один раз, положить только в `.env` машины
(не в репозиторий), хранить авторитетную копию в менеджере паролей оператора. Сгенерировать пару:

```bash
npx web-push generate-vapid-keys
```

(или любой другой генератор, совместимый с RFC 8292 — важно, чтобы приватный ключ реально
...
> каждая существующая подписка (`PushSubscription`, сохранённая в браузере мастера) была выдана
> push-сервисом ПОД СТАРЫЙ публичный ключ, и после смены пары push-сервис перестаёт принимать от
> нас сообщения по этим подпискам. Другими словами — **потеря или смена пары обнуляет все
> существующие подписки разом**, без исключений и без возможности восстановить их той же парой
> заново; единственный путь назад — каждый мастер открывает у себя переключатель уведомлений и
> подписывается заново (это UI-действие на пару кликов, не требует поддержки).
>
> **План действий при намеренной смене (ключ засветился, утёк из чужой копии `.env` и т.п.):**
> 1. Сгенерировать новую пару тем же способом, что и выше.
> 2. Заменить `WEBPUSH_VAPID_PUBLIC_KEY`/`WEBPUSH_VAPID_PRIVATE_KEY` в `.env` на новые значения
>    (в отличие от §11.3 здесь не нужен отдельный `*_ACK` — нет fail-fast по отпечатку, потому что
>    нет данных, отпечаток которых можно было бы не совпасть).
>  3. `docker compose -f docker-compose.prod.yml --env-file .env up -d api` — применить.
> 4. Существующие серверные записи `PushSubscription` можно оставить как есть: первая попытка
>    доставки по каждой вернёт от push-сервиса `410 Gone`/`404`, и это тот же путь очистки,
>    который уже предусмотрен обычным протуханием подписок (dispatch помечает их для удаления по
>    ответу провайдера) — отдельной ручной миграции данных под смену ключа не требуется.
> 5. Сообщить об этом факте отдельно (например, в CHANGELOG/объявлении), потому что мастера
>    увидят молчание уведомлений без явной ошибки — переключатель в профиле останется на вид
>    «включён», просто сообщения перестанут доходить, пока мастер не переподпишется вручную.

## 18. Подтверждение телефона через бота MAX (цикл 14 — ARCHITECTURE_CYCLE14.md §150)

### 18.1. Что выкачено и в каком состоянии

Подсистема выкачена **выключенной**: `PHONEVERIFY_PROVIDER=stub`. В этом состоянии
`GET /api/phone-verification/config` отвечает `enabled=false`, кнопка «Подтвердить номер через MAX»
не появляется ни на регистрации, ни в профиле, четыре секрета ниже не требуются, и сборка зелёная
без них. Это ожидаемое состояние после выката, а не недоделка — ровно как WhatsApp-канал (цикл 4) и
Web Push (цикл 9).

⚠️ **Одно следствие, которое видно пользователю уже сейчас, при выключенной подсистеме.**
`POST /api/profile/change-phone` на номер, с которого уже есть гостевые записи, отвечает **409**
(`ChangePhoneSubsystemDisabled`) — то есть такая смена номера не выполняется вовсе, пока
подтверждение не включено. Это осознанный выбор в пользу приватности гостя: до цикла 14 смена
проходила молча и открывала доступ к чужой истории визитов.

### 18.2. Что нужно, чтобы включить

Предусловие, которое не делается из кода: **бот в MAX должен существовать**. Заводится через
`@MasterBot` в мессенджере владельцем верифицированного бизнес-профиля MAX; ник обязан
заканчиваться на `_bot`. Без него включать нечего.

Дальше — пять переменных из `.env.production.example` (блок «Подтверждение телефона через MAX»):

| Переменная | Что это | Где взять |
|---|---|---|
| `PHONEVERIFY_PROVIDER` | `stub` → `max-bot` | это и есть рубильник |
| `PHONEVERIFY_MAX_BOT_TOKEN` | токен бота; он же — ключ HMAC для проверки подписи контакта | выдаёт `@MasterBot` при создании |
...
подсистема хуже выключенной.

⚠️ **Не переиспользуйте здесь секреты уведомлений.** `NOTIFICATIONS_ENCRYPTION_KEY` и
`NOTIFICATIONS_GREENAPI_MAX_PARTNER_TOKEN` — другая подсистема, другой жизненный цикл, другой
владелец. Совпадение слова «MAX» в названиях ничего не значит: там канал уведомлений салона через
GREEN-API, здесь — платформенный бот.

### 18.3. Требования MAX к вебхуку — проверить до включения

1. **Только HTTPS на 443, сертификат доверенного УЦ.** Самоподписанный и HTTP MAX отключил;
   вебхук на нестандартном порту он не примет.
2. **Подписка живёт 8 часов без успешного ответа и снимается сама.** Поэтому в приложении есть
   шестая регулярная задача `max-webhook-renew`, которая переподписывается при старте и дальше по
...
### 18.4. Как убедиться, что включилось

```bash
curl -s https://ezbook.ru/api/phone-verification/config | jq
```

Ожидаемо `enabled: true` и имя бота. Дальше — диагностика для суперадмина:

```bash
curl -s -H "Authorization: Bearer <токен суперадмина>" \
  https://ezbook.ru/api/admin/phone-verification/diagnostics | jq
```

Она показывает состояние подписки на вебхук и время последнего успешного обновления — это первое
место, куда смотреть, если подтверждения перестали доходить.

Живая проверка руками: открыть регистрацию, нажать «Подтвердить номер через MAX», пройти по ссылке
(на десктопе — сканировать QR), в боте нажать «Поделиться контактом». Телефон, которым делитесь,
должен совпадать с введённым в форме — иначе подсистема честно откажет, это её работа.

### 18.5. Чего в подсистеме нет намеренно

**Кодов подтверждения нет и не будет.** Лицензия MAX (ред. 27.08.2026) запрещает использовать
чат-бота для формирования, передачи и отправки OTP-кодов и «Авторизационных сообщений». Если
кто-то предложит «просто присылать код ботом» — это не упрощение, а нарушение лицензии с риском
потерять бота. Платные способы (звонок, SMS) запланированы отдельным циклом и пойдут через порт
`IPhoneVerificationMethodAdapter`, а не через бота.

## 19. Проверка адреса геокодером (цикл 13 — ARCHITECTURE_CYCLE13.md §206/§213/§216)

### 19.0. ⚠️ Предусловие перед выкатом этого релиза: цикл 13 добавляет правовой ключ

Цикл 13 вводит седьмой интерфейсный правовой текст — `uiTexts` ключ **`PublicAddressNotice`**
(предупреждение владельцу о том, что адрес компании публикуется открыто). Это **меняет состав
ключей** в `legal.json`, то есть попадает под [шаг 10.2c](#шаг-102c), а не под 10.2b: манифест на
машине надо привести в соответствие **до** выката кода, иначе релиз не встанет.

Проверено на практике 2026-09-24: `deploy-staging.yml` остановился на своей же первой предпроверке
с сообщением `missing uiTexts key(s) in legal/legal.json: PublicAddressNotice`, **не тронув на машине
ничего** — откат не требовался. Это ожидаемое поведение предпроверки, а не поломка деплоя.

Порядок действий — как в §10.2c: сначала обновить `/opt/ezbook/app/legal/` (штатно —
`legal-tools publish`, см. §US-11-08; ручная правка `legal.json` на сервере ничего не проверяет и
оставляет комплект в непубликуемом виде незамеченно), затем катить код. Каталог `legal/` деплой
намеренно никогда не перезаписывает — там подставленные реквизиты оператора.

### 19.1. Что выкачено и в каком состоянии

Подсистема выкачена **выключенной**: `ADDRESSVERIFICATION__PROVIDER=logging`. В этом состоянии
`POST /api/companies/address/lookup` отвечает **404**, `CompanyDto.addressVerification.available =
false`, кнопка проверки адреса в интерфейсе не рисуется вовсе (не «рисуется неработающей»), ни
одного сетевого вызова не выполняется. **Это штатное состояние прода после цикла 13, а не
недоделка** — ровно как выключенный WhatsApp-канал (цикл 4), Web Push (цикл 9) и подтверждение
телефона через MAX (цикл 14, §18). Ссылки-переходы на Яндекс Карты и 2ГИС рядом с адресом компании
(§200 п. 2 ARCHITECTURE_CYCLE13.md) от этого рубильника не зависят и работают всегда — они не требуют
ни ключа, ни лицензии.

Поле «Адрес» при этом полностью рабочее и без рубильника: `PUT /api/companies/{id}` сохраняет то,
что ввёл владелец, статус — `Unverified`.

### 19.2. Что нужно, чтобы включить

Предусловие, которое не делается из кода: **куплен платный ключ геокодера Яндекса.** Бесплатного или
триального режима для продакшена нет. Вариант лицензии (стандартная / расширенная Яндекса /
адресный сервис на ФИАС-ГАР) и то, на кого именно она оформляется, — уточнить **у продавца
лицензии**, а не по статьям реселлеров (ARCHITECTURE_CYCLE13.md §206/§213): в документации к этому
циклу сознательно нет ни одного утверждения вида «лицензия только на юрлицо/ИП» — это не
установленный факт.

Пять переменных из `.env.production.example` (блок «Проверка адреса геокодером»):

| Переменная | Что это | Где взять |
|---|---|---|
| `ADDRESSVERIFICATION__PROVIDER` | `logging` → `yandex` | это и есть рубильник |
| `ADDRESSVERIFICATION__YANDEX__APIKEY` | ключ геокодера, серверный секрет | выдаётся при покупке лицензии |
| `ADDRESSVERIFICATION__STORERESULTS` | `false` → `true` | **отдельный рубильник, см. 19.3 — не переключать вместе с провайдером бездумно** |
| `ADDRESSVERIFICATION__MAXCANDIDATES` | сколько вариантов адреса показывать (1–5) | обычно оставить `5` |
| `ADDRESSVERIFICATION__CACHEHOURS` | TTL кеша геокодера, часы | обычно оставить `24`, см. 19.3 про потолок |

При `ADDRESSVERIFICATION__PROVIDER=yandex` без `ADDRESSVERIFICATION__YANDEX__APIKEY` API **не
стартует** вне Development (`DeploymentSafetyChecks.ValidateAddressVerification`) — наполовину
включённая подсистема хуже выключенной, как и в §18.2. Нераспознанное значение `Provider` роняет
старт **всегда**, в любом окружении.

### 19.3. `StoreResults` и `CacheHours` — лицензионные ограничения, не настройки производительности

⚠️ Это не пара обычных флагов тюнинга. Оба значения существуют потому, что стандартная лицензия
Яндекс Геокодера **запрещает хранить результат запроса** — ни координаты, ни даже нормализованную
строку адреса, — и разрешает лишь временное кеширование не дольше 30 суток.

- **`ADDRESSVERIFICATION__STORERESULTS=true`** можно включать **только** при купленной **расширенной**
  лицензии Яндекс Геокодера (та, что явно допускает сохранение результата). Под стандартной лицензией
  это значение обязано оставаться `false` — код не может проверить, что лицензия действительно
  расширенная, поэтому при `StoreResults=true` приложение лишь пишет предупреждение в лог при
  старте, не блокируя его. Ответственность за то, что лицензия соответствует значению, — на том, кто
  включает.
- **`ADDRESSVERIFICATION__CACHEHOURS`** обязан оставаться **≤ 720** (30 суток) — это не техническая
  рекомендация, а прямое выражение лицензионного потолка (`LEGAL_REVIEW.md` §16.2/§16.5). Значение
  вне диапазона `[0, 720]` роняет старт API **всегда**, в любом окружении, включая Development — это
  единственная проверка в этой подсистеме, которая не смягчена вне Production.

### 19.4. Пункт 9.8 политики конфиденциальности — публикуется в тот же момент, что и рубильник

**Это не два независимых пункта чек-листа, а одна пара.** П. 9.8 политики (`LEGAL_REVIEW.md` §16.3
б) описывает передачу строки адреса и названия города правообладателю геокодера при проверке —
такая передача до включения `Provider=yandex` физически не происходит, и публиковать текст о ней
раньше нельзя (текст был бы неправдой), а публиковать позже — значит какое-то время выполнять
передачу, о которой политика молчит. П. 9.7 (переходы на карты) уже опубликован в цикле 13 и
включения рубильника не ждёт — он не про эту подсистему.

**Полный порядок включения (ARCHITECTURE_CYCLE13.md §216), по шагам, строго в этом порядке:**

1. Выбран вариант лицензии (стандартная Яндекса / расширенная Яндекса / адресный сервис на
   ФИАС-ГАР) — см. 19.2.
2. Лицензия куплена; лицо и вид лицензии уточнены у продавца.
3. Если планируется `ADDRESSVERIFICATION__STORERESULTS=true` — проверено, что лицензия именно
   **расширенная** (см. 19.3).
4. **Текст п. 9.8 политики конфиденциальности опубликован** (`legal-drafts/01-privacy-policy.html`
   и его копия в `App_Data/legal/`, `LEGAL_REVIEW.md` §16.3 б; задача L3.2 ARCHITECTURE_CYCLE13.md
   §217) — не раньше и не позже следующего шага, а **в тот же деплой**.
5. `ADDRESSVERIFICATION__CACHEHOURS` проверен на ≤ 720.
6. Только теперь `ADDRESSVERIFICATION__PROVIDER=yandex`, ключ проставлен, деплой/перезапуск.

### 19.5. Как убедиться, что включилось

```bash
curl -s -X POST https://ezbook.ru/api/companies/address/lookup \
  -H "Authorization: Bearer <токен владельца компании>" -H "Content-Type: application/json" \
  -d '{"companyId": "<id>", "query": "Ленина 5"}' | jq
```

Ожидаемо — не 404, а список вариантов адреса. Живая проверка руками: в кабинете компании открыть
поле «Адрес», убедиться, что появилась кнопка проверки и предупреждение о публичности адреса рядом
с ней (последнее показывается независимо от рубильника, оно не про геокодер).
